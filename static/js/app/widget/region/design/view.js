define(
		["text!app/widget/region/design/template/region_design.html",
				"text!app/widget/region/design/template/view_graph.html",
				"text!app/widget/region/design/template/view_filter.html",
				"text!app/widget/region/design/template/view_property.html",
				"text!app/widget/region/design/template/dialog_edit_x.html",
				"text!app/widget/region/design/template/dialog_edit_y.html"],
		function(regionDesignTemplate, graphViewTemplate, filterViewTemplate,
				propertyViewTemplate, dialogEditXTemplate, dialogEditYTemplete) {
			var DesignRegionView = DataInsightManager.module("DesignRegion",
					function(DesignRegion, DataInsightManager, Backbone,
							Marionette, $, _) {
						...
						DesignRegion.Filter = Marionette.ItemView.extend({
							...
							onRender : function() {
								var self = this;
								var whichColumn = this.model.get("whichColumn")
										+ 1;
								// this.$el.find(".myfilter li:nth-child(" +
								// whichColumn +")").addClass("select-filter");
								$(".myfilter li:nth-child(" + whichColumn + ")")
										.addClass("select-filter");
								// 当字段拖到过滤器的时候触发
								this.$el.find(".myfilter").droppable({
									drop : function(event, ui) {
										var axisItem = ui.helper
												.data("axisitem");
										// 通知model获取fieldName字段的所有值的集合
										self.model.trigger(
												"fetch:field:values", {
													name : axisItem.name,
													table : axisItem.table,
													kind : axisItem.kind
												});
									}
								});
								// 监听选中过滤器中的值
								this.$el
										.find(".myfilter-values input[type=checkbox]")
										.click(function() {
											if ($(this).prop("checked")) {
												...
												self.model.trigger(
														...
														$(this).val());
												self.$el
														.find(".myfilter-select-or-not li:contains('["
																+ $(this).val()
																+ "]')")
														.remove();
											}
								...
								// 当操作改变的时候触发
								this.$el.find(".select-or-not").on("change",
										function() {
											if (self.$el
													.find(".myfilter-select-or-not li").length > 0) {
												// 通知model值是选中还是排除
												self.model.trigger(
														"operate:change",
														$(this).val());
								...
								// 当清空选中的时候触发
								this.$el.find(".clear-select").on("click",
										function() {
											if (self.$el
													.find(".myfilter-select-or-not li").length > 0) {
												// 通知model清空选中的值
												self.model
														.trigger("values:clear");
												self.$el
														.find(".myfilter-select-or-not li")
														.remove();
												self.$el
														.find(".myfilter-values li input[type=checkbox]")
														.prop("checked", false);
											}
										})

								// 当点击清空所有的过滤器的时候触发
								this.$el.find(".clear-filters").on("click",
										function() {
											if (self.$el.find(".myfilter li").length > 0) {
												// 通知model清空所有的过滤器
												self.model
														.trigger("filters:clear");
												self.$el.find(".myfilter li")
														.remove();
												self.$el
														.find(".myfilter-select-or-not li")
														.remove();
												self.$el
														.find(".myfilter-values li")
														.remove();
											}
										})

								// 选中某个过滤器时候触发
								this.$el.find(".myfilter>li").on("click",
										function() {
											var whichFilter = $(this).index();
											if (whichFilter != self.model
													.get("whichColumn")) {
												// 通知model选中过滤器
												self.model.trigger(
														"select:filter",
								...
								// 当删除选中过滤器的时候触发
								this.$el.find(".myfilter>li>b").on("click",
										function() {
											var whichFilter = $(this).parent()
													.index();
											// $(this).parent().remove();
											// 通知model删除选中过滤器
											self.model.trigger("filter:remove",
													whichFilter);
										});

								// 监听过滤器是数值变量的时候输入最大最小值的输入
								this.$el.find("#lowRange").on("change",
										function() {
											...
											// 通知model获取数值变量过滤器的下限
											self.model.trigger("lowRange:add",
													lowRange);
										})
								this.$el.find("#hightRange").on("change",
										function() {
											...
											var lowRange = self.$el
													.find("#lowRange").val();
											if (lowRange
													&& Number(heightRange) < Number(lowRange)) {
												$(this).val("");
												// alert("最大值不能小于最小值");
												return;
											}
						...
						DesignRegion.Property = Marionette.ItemView.extend({
									...
									onShow : function() {
										// 显示值
										$("[name=style]").val(this.model
												.get("style"));
										$("[name=autoRefresh]").val(this.model
												.get("autoRefresh"));
										$("[name=isPublish]").val(this.model
												.get("isPublish").toString());
										// name和title一样
										$("[name=title]").val(this.model
												.get("name"));
									...
									whenChange : function() {
										...
										// 监听更新频率下拉框的变化
										$("[name=autoRefresh]").on("change",
												function() {
													// 通知property
													// model更新autoRefresh属性
													self.model
															.trigger(
																	"autoRefresh:change",
																	$(this)
										...
										/*
										 * $("[name=title]").bind("input
										 * propertychange", function(){
										 * console.log($(this).val()); })
										 */
										// 监听标题输入框的值的改变
										$("[name=title]").on("change",
												function() {
													self.model.trigger(
															"title:change",
															$(this).val());
						...
						DesignRegion.Graph = Marionette.ItemView.extend({
							...
							initialize : function() {
								/*
								 * //当改变图表类型的时候触发，更新图表视图
								 * this.model.on("feature:change",
								 * function(mapping){
								 * this.render();//重新render后等于新建视图，则在onShow方法绑定的事件失效 },
								 * this); //当x、y轴属性改变的时候触发，更新图表视图
							...
							onShow : function() {
								...
								// 监听x轴y轴的操作
								$("#x_sortable").on("click",
										".coordinate-remove a", function(e) {
											var xItem = $(e.target)
													.data("xitem");
											self.model.trigger("x:remove",
													xItem);
											$(e.target).parents(".measure")
													.remove();
								...
								$("#y_sortable").on("click",
										".coordinate-remove a", function(e) {
											var yItem = $(e.target)
													.data("yitem");
											self.model.trigger("y:remove",
													yItem);
											$(e.target).parents(".mension")
													.remove();
											self.enableFiledDrage(yItem.name);
										});
								// 监听编辑x轴y轴中元素编辑的操作
								$("#x_sortable").on("click",
										".coordinate-edit a", function(e) {
											var xItem = $(e.target)
													.data("xitem");
								...
								/*
								 * $.each(this.model.get("x"), function(index,
								 * value){ self.disableFiledDrage(value.name);
								 * }); $.each(this.model.get("y"),
							...
							setDragProperty : function(e) {
								...
								$("#x_sortable,#y_sortable").sortable({
									...
									receive : function(event, ui) { // 这个事件在一个已连接的sortable接收到来自另一个列表的元素时触发.
										// 判断x轴还是y轴的标志
										var axis = $(this).attr("id") == "x_sortable"
												? "x"
												: "y";
										// 坐标某个元素的序列化
										...
										// 显示标题
										var title = axisItem.title == undefined
												? name
												: axisItem.title;
										// 计算函数
										var calcFunc = axisItem.calcFunc == undefined
												? "none"
												: axisItem.calcFunc;
										...
										if (axis == "x") {
											...
											self.model.trigger("x:add",
													addXItem);
										} else {
											// 通知graph model添加y轴的这个被拖进来的元素
											self.model.trigger("y:add",
													addXItem);
										}
									},
						...
						DesignRegion.DialogX = Marionette.ItemView.extend({
							...
							onShow : function() {
								...
								this.$el.find("#save-setting").click(
										function() {
											var data = {
												title : $("[name=x_title]")
														.val(),
												calcFunc : $("[name=x_calcFunc]")
														.val(),
												kind : $("[name=x_type]").val(),
												axis : "x"
											}
											// 触发axis:change事件，通知axis model更新
											self.model.trigger("axis:change",
													data);
										})
							},
							/*
							...
							onBeforeShow : function() {
								this.$el.find("[name=x_calcFunc]")
										.val(this.model.get("calcFunc"));
								this.$el.find("[name=x_type]").val(this.model
										.get("kind"));
							}
						});
						...
						DesignRegion.DialogY = Marionette.ItemView.extend({
							 ...
							 */
							onBeforeShow : function() {
								this.$el.find("[name=y_calcFunc]")
										.val(this.model.get("calcFunc"));
								this.$el.find("[name=y_type]").val(this.model
										.get("kind"));
							}
						})
					})
