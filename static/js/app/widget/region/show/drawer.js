define(	["echarts", "echarts/chart/bar", "echarts/chart/line",
				...
				"echarts/chart/radar", "echarts/chart/table", "tool"],
		function(echart) {
			...
			var DrawManager = function(opt) {
				...
				this.run = function(place, resp, dynamicObj) {
					...
					if ("map" !== type) {
						...
						var self = this;
						require(["echarts/chart/map", "echarts/config",
										"common/city"], function(_m, ecConfig,
										_t) {
					...
					if (this.updator.isEnable())
						this.updator.restart()
				};
			...
			var AxisDrawer = function() {
				...
				/*
				 * this.catStyle = { boundaryGap : false };
				 * 
				 * this.valStyle = { axisLabel : { formatter: '' } , splitArea :
				 * {show : true} };
				 */

				this.seriesNewAdd = [0, 0, true, false, null // 参数意义见echart官网
				];
				...
				this.drawNewAdded = function(data) {
					...
					$.each(data['le_val'], function(i, v) {
								...
								idx = (v['le'] === '') ? 0 : self
										.findSeriesIdxByName(v['le']);
								newAddData[0] = idx;
								newAddData[1] = v['val'];
								newAddData[4] = data['cat']
										&& data['cat'].length >= i
										? data['cat'][i]
										: null;
			...
			var TableDrawer = function() {
				...
				this.fillSeries = function(data) {
					...
					var columnPosList = $.map(data["belong_column"],
							function(i) {
								return self.estColumnPos(i)
							});

					for (var k in data) {
						if (k !== "row" && k !== "belong_row" && k != "column"
								&& k != "belong_column") {
			...
			var MapDrawer = function() {
				...
				this.fillSeries = function(data) {
					...
					if (data.hasOwnProperty("line_value")) {
						...
						this.seriesOne.markPoint.data = $.map(
								data["line_value"], function(one) {
									return one[1]
								})
					}
