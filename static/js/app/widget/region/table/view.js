/*
 * ! table模板定义 Date: 2014-7-25
 */
define(
		[
				"text!app/widget/region/table/template/region_table.html",
				"text!app/widget/region/table/template/dialog_new_table.html",
				"text!app/widget/region/table/template/dialog_choosed_db.html",
				"text!app/widget/region/table/template/dialog_import_file.html",
				"text!app/widget/region/table/template/dialog_connect_db.html",
				"text!app/widget/region/table/template/dialog_table_manage.html"],
		function(tableRegionTemplate, newTableTemplate, choosedDbTemplate,
			...
			var data = DataInsightManager.module("TableRegion",
					function(TableRegion, DataInsightManager, Backbone,
							Marionette, $, _) {
						...
						TableRegion.TableView = Marionette.ItemView.extend({
							...
							onShow : function() {
								...
								// 设置高度
								variable = DataInsightManager.tableRegion.$el
										.outerHeight()
										- this.$("#table_template_header")
												.outerHeight()
								this.$("#table_template_content")
										.height(variable);

								// 三种不同情况下显示数据表（无数据，有数据有选中表，有数据无选中表）
								selectedModelList = this.collection.where({
											selected : true
										});
								if (selectedModelList.length > 0) {
									choosedModel = this.collection.findWhere({
												choosed : true
											});
									if (!choosedModel) {
										this.$(".table-item").eq(0)
												.addClass("table-item-choosed");
									}
									DataInsightManager.dialogRegion.trigger(
											'pass:selected-table',
											selectedModelList);
								} else {
									DataInsightManager.dialogRegion.trigger(
											"change:fields", []);
									DataInsightManager.dialogRegion.$el
											.modal("hide");
								}
							},
							...
							changeSelectTable : function(e) {
								this.$(".table-item-choosed")
										.removeClass("table-item-choosed");
								$(e.target).addClass("table-item-choosed");

								var choosedModel = this.collection.findWhere({
											choosed : true
										});
								DataInsightManager.dialogRegion.trigger(
										'model:set', choosedModel, {
											"choosed" : false
										});

								var id = $(e.target).attr("data-id");
								DataInsightManager.dialogRegion.trigger(
										'model:set', this.collection.get(id), {
											"choosed" : true
										});
								...
								DataInsightManager.dialogRegion.trigger(
										"change:fields", fields, tableName);
							}
							,
						 ...
						 */
						TableRegion.newTableDialog = Marionette.ItemView
								.extend({
									tagName : "div",
									...
									newTabelDbFunction : function() {
										DataInsightManager.dialogRegion
												.trigger('show:dialog-choosed-db');
									},
									newTableFileFunction : function() {
										DataInsightManager.dialogRegion
												.trigger('show:dialog-import-file');
									}
									,
								});
						 ...
						 */
						TableRegion.choosedDbDialog = Marionette.ItemView
								.extend({
									tagName : "div",
									className : "modal-dialog",
						 ...
						 */
						TableRegion.connectDbDialog = Marionette.ItemView
								.extend({
									tagName : "div",
									className : "modal-dialog",
									...
									managetableFunction : function() {
										...
										var options = {
											...
											"port" : this.$("#connect_port")
													.val(),
											"db" : this.$("#connect_db").val(),
											"user" : this.$("#connect_user")
													.val(),
											"pwd" : this.$("#connect_pwd")
													.val(),
											"kind" : this.$("#connect_kind")
													.val()
										};
										this.$(".connect-db-commit")
												.html("连接中...");
										this.$(".connect-db-commit").css(
												"cursor", "wait");
										DataInsightManager.dialogRegion
												.trigger('connect:get-data',
														this.model, options);
									},
						 ...
						 */
						TableRegion.importFileDialog = Marionette.ItemView
								.extend({
									tagName : "div",
									className : "modal-dialog",
									...
									managetableFunction : function() {
										...
										this.$(".import-file-commit")
												.html("导入中...");
										this.$(".import-file-commit").css(
												"cursor", "wait");
										var options = {
											"type" : this
													.$("#import_file_type")
						 ...
						 */
						TableRegion.tableManageDialog = Marionette.ItemView
								.extend({
									tagName : "div",
									className : "modal-dialog",
									...
									tableListFunction : function() {
										this.$(".table-manage-commit")
												.html("加载中...");
										this.$(".table-manage-commit").css(
												"cursor", "wait");
										var self = this;
										this.$(".manageCheckbox").each(
												function() {
									...
									toggleCheckedFunction : function(ev) {
										if ($(ev.currentTarget).find("input")
												.attr("checked")) {
											$(ev.currentTarget).find("input")
													.removeAttr("checked");
										} else {
											$(ev.currentTarget).find("input")
													.attr("checked", true)
