define(	["text!app/widget/region/switch/template/region_switch.html",
				"text!app/widget/region/switch/template/region_switchs.html",],
					...
					function(SwitchRegion, DataInsightManager, Backbone,
						...
						SwitchRegion.Area = Marionette.ItemView.extend({
							...
							onShow : function() {
								var self = this;
								$(".selected-workbook")
										.removeClass("selected-workbook");
								this.$el.addClass("selected-workbook");
								// 监听切换工作区
								this.$el.on("click", function() {
											if ($(this)
													.hasClass("selected-workbook"))
												return;
											$(".selected-workbook")
													.removeClass("selected-workbook");
											$(this)
													.addClass("selected-workbook");
											// 通知controller去切换工作区
											self.trigger("area:switch",
													self.$el.index());
										});
						...
						SwitchRegion.Areas = Marionette.CompositeView.extend({
									...
									onShow : function() {
										console.log("show");
										// 监听修改组件名字
										this.collection.on("name:change",
												function(name) {
