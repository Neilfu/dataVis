...
define(
		['app/widget/region/show/controller',
				...
				'app/widget/region/switch/view', 'app/widget/entities/switch',],
		function() {
			var SwitchController = DataInsightManager.module("SwitchRegion",
					function(SwitchRegion, DataInsightManager, Backbone,
							Marionette, $, _) {
						...
						SwitchRegion.Controller = function() {
							var obj = {
								initialize : function() {
									...
									this.collection = DataInsightManager
											.request("switch:entities");
									this.switchViews = new SwitchRegion.Areas();
									/*
									 * 下面的三个监听一定要放在this.switchViews赋值后
									 */
									// 监听新建工作区
									this.switchViews.on("switch:new",
											function() {
												window.newArea = true;// 防止在编辑的时候新建工作区会从编辑的组件中获取数据
												this.newArea();
											}, this);
									this.switchViews.on("switch:remove",
											function(i) {
												// 删除前,选中隔壁的工作区
												if (i > 0) {
													// this.switchArea(i - 1);
												...
												this.designControllerList
														.remove(removeDesignController);
												var removeShowController = this.showControllerList[i];
												this.showControllerList
														.remove(removeShowController);
												this.collection
									...
									this.switchViews.on(
											"childview:area:switch", function(
													childView, i) {
												this.switchArea(i);
											}, this);
									// 默认新建一个工作簿
								...
								newArea : function() {
									...
									$.when(fetchDesingeController).done(
											function(desingeController) {
												desingeController
														.showDesingView();
												self.designControllerList
														.push(desingeController);
												var switchEntity = DataInsightManager
														.request("switch:entity");
												switchEntity
														.set(
																"name",
																desingeController.property
																		.get("name"));
												self.collection
														.add(switchEntity);
												self.switchViews.collection = self.collection;
												// 显示工作簿
												DataInsightManager.switchRegion
														.show(self.switchViews);
											})

									var showController = new DataInsightManager.ShowRegion.Controller();
									showController.showShowView();

									self.showControllerList
											.push(showController);
								},
								/**
								 * 切换工作簿
