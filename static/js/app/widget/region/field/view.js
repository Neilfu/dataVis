/*
 * ! field模板定义 Date: 2014-7-28
 */
define(
		["text!app/widget/region/field/template/region_field.html",
				"text!app/widget/region/field/template/dialog_field_manage.html"],
		function(fieldRegionTemplate, fieldManageTemplate) {

			var data = DataInsightManager.module("FieldRegion",
					function(FieldRegion, DataInsightManager, Backbone,
							Marionette, $, _) {
						...
						FieldRegion.FieldView = Marionette.ItemView.extend({
							...
							onShow : function() {
								var self = this;
								var variable = DataInsightManager.fieldRegion.$el
										.outerHeight()
										- this.$("#field_template_header")
												.outerHeight()
								this.$("#field_template_content")
										.height(variable);
								...
								this.$(".field-item").draggable({
									...
									helper : function(event) {
										return $("<li class='dragging-field-item' data-filedname='"
												+ $(this).data("filedname")
												+ "' data-axisitem='"
												+ JSON.stringify($(this)
														.data("axisitem"))
									...
									// 所有的回调函数(start, stop, drag)接受两个参数:
									// 浏览器事件和ui对象
						...
						FieldRegion.FieldManageDialog = Marionette.ItemView
								.extend({
									tagName : "div",
									...
									fieldManageCommitFunction : function(e) {
										// 获取信息
										var options = $("#field-manage-form")
												.serialize();
										// 中文会被编码，所以需要解码
										options = decodeURIComponent(options,
												true)
										this.trigger('change:field-attributes',
												options);
									},

									changeTextFunction : function(e) {
										$(e.target).parents("ul")
												.find(".field-change-flag")
												.val("1");
									},
