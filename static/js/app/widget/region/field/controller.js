/*
 * ! field控制器 Date: 2014-7-25
 */
define(['app/widget/entities/field', 'app/widget/region/field/view'], function(
		f, v) {

	var data = DataInsightManager.module("FieldRegion", function(FieldRegion,
			DataInsightManager, Backbone, Marionette, $, _) {
		FieldRegion.Controller = {
			ListFields : function(fields, tableName) {

				// 获取数据
				var fieldsCollection = DataInsightManager.request(
				...
				fieldsListView.on("show:manage-dialog", function() {
					...
					fieldManageView.on("change:field-attributes", function(
							options) {
						// 解析数据
						if (tableName) {
							var i, temp, j = -1, dataList = [], flag, arrList = options
									.split("&");
							...
							if (flag) {
								...
								DataInsightManager.trigger("showField",
										fieldsCollection.models, tableName);
