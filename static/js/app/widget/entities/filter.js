 ...
 * 
 */
define(["app/widget/entities/entrance"], function() {

	var FilterEntity = DataInsightManager.module("Entities", function(Entities,
		...
		Entities.Filter = Backbone.Model.extend({
			defaults : {
				filters : [],// 过滤器列表,形式[{table: xxx, name: xxx, kind:
								// xxx},{table: xxx, name: xxx, kind:
								// xxx}],table为表名，name为字段名, kind为数值类型
			...
			initialize : function() {
				...
				this.on("fetch:field:values", function(data) {
							...
							this.set("whichColumn", columnsNumber, {
										silent : true
									});
							// 如果是因子变量，则到后台抓取所有的数据
							if (data.kind == 'F') {
								this.push("values",
										new Entities.filterAssist(), {
											silent : true
										});
				...
				this.on("filter:select", function(value) {
					...
					if (this.get("operators")[this.get("whichColumn")] == "include") {
						this.get("values")[this.get("whichColumn")].push(
								"inValues", value);
					}
					// 排除
				...
				this.on("filter:notselect", function(value) {
					// 选中
					if (this.get("operators")[this.get("whichColumn")] == "include") {
						this.get("values")[this.get("whichColumn")]
								.get("inValues").remove(value);
					}
					// 排除
					else if (this.get("operators")[this.get("whichColumn")] == "exclude") {
						this.get("values")[this.get("whichColumn")]
								.get("exValues").remove(value);
					}
					// 通知入口model过滤器改变
					Entities.trigger("filter:change", this.toJSON());
				})
				...
				this.on("operate:change", function(operate) {
					if (this.get("filters").length > 0) {
						// 之前排除，现在选中的情况
						if (operate == "include") {
							var beforeExValues = this.get("values")[this
									.get("whichColumn")].get("exValues");
							this.get("values")[this.get("whichColumn")].set(
									"inValues", beforeExValues);
							this.get("values")[this.get("whichColumn")].set(
									"exValues", []);
						} else if (operate == "exclude") {// 之前选中，现在排除的情况
							var beforeInValues = this.get("values")[this
									.get("whichColumn")].get("inValues");
							this.get("values")[this.get("whichColumn")].set(
									"exValues", beforeInValues);
							this.get("values")[this.get("whichColumn")].set(
									"inValues", []);
						}
				...
				this.on("select:filter", function(whichFilter) {
							this.set("whichColumn", whichFilter, {
										silent : true
									});
			...
			/**
			 * 通知wiget模型去后台fetch数据后代理执行的函数 data:后台返回的response，
			 * defer：fetchFromWidget函数里面的jquery deferred
			 */
			handlerData : function(data, defer) {
				this.filter = data.filter;
				// this.filter = "color = 'blue' or color = 'red' and size=1 or
				// size=2";
			...
			/**
			 * 说明：如果是编辑状态，则在抓取完后台数据后再监听change事件,否则直接监听 return
			 * ture对应编辑状态，false对应创建状态
			 */
			listenChange : function() {
				var self = this;
				/*
				 * //编辑状态 if(window.widgetId && !window.newArea){
				 * //确保从后台抓取完数据后才监听属性改变事件，确保不会做无谓的触发
				 * $.when(this.fetchFromWidget()).done(function(){
				 * //只要模型的属性改变便通知widget模型改变属性 self.on("change", function(){
				 * Entities.trigger("filter:change", this.toJSON()); //通知视图重绘
				 * this.trigger("filter:rerender"); }, self); }); window.newArea =
				 * true;//防止在编辑的状态下，新建组件会从编辑的组件那里获取数据 return true; }
				 */
				// 创建状态
