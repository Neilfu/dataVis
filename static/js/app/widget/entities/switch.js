...
define([], function() {
			...
			var SwitchEntity = DataInsightManager.module("Entities", function(
							Entities, DataInsightManager, Backbone, Marionette,
							$, _) {
						...
						Entities.Switchs = Backbone.Collection.extend({
									...
									initialize : function() {
										/*
										 * 监听组件名字改变，在组件属性中，修改名字的时候会触发，由property
										 * model触发
										 * 注意是绑定在Entities上，所以下面触发的事件肯定是要绑定在switch的CollectionView的
										 */
										Entities.on("name:change", function(
														name) {
						...
						DataInsightManager.reqres.setHandler("switch:entity",
								function() {
									return API.getSwitchEntity();
								});

						DataInsightManager.reqres.setHandler("switch:entities",
								function() {
