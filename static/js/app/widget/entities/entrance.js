define(	['app/widget/entities/graph', 'app/widget/entities/filter',
				'app/widget/entities/property', 'app/widget/common/mapping'],
		function() {
			DataInsightManager.module("Entities", function(Entities,
				...
				Entities.AdditionalEntrance = Entities.BaseEntrance.extend({
							initialize : function() {
								this.interval = null;
								_.bindAll(this, "changeRefresh", "changeStyle",
										"onEditWidget")
							},

							listen : function(name) {
								Entities.on(name, this.onEditWidget);
								Entities.on("autoRefresh:change",
										this.changeRefresh);
								Entities.on("style:change", this.changeStyle)
							},

							onEditWidget : function(data) {
								if ("0" != data.autoRefresh)
									this.changeRefresh(data)
								if ("default" != data.style)
							...
							changeStyle : function(data) {
								DataInsightManager.commands.execute(
										"style:try", data.style)
							}
						});
				...
				Entities.EntranceFascade = Backbone.Model.extend({
					...
					initialize : function() {
						...
						this.adtEntrance.on("refresh:notice",
								this.refreshEntrance.reqRefresh);

						Entities.on("design:initial", $.proxy(
										this.onReqWidgetData, this));
					},
			...
			(function() {
				...
				Entities.entAPI = {
					setRelation : function(kind, model, changeEvent) {
						Entities.entranceFascade.classify(kind, model,
								changeEvent)
					},
				...
				DataInsightManager.reqres.setHandler("entrance:entity",
						function() {
