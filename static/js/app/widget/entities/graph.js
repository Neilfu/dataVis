...
define(["app/widget/entities/entrance"], function() {
	var GraphEntity = DataInsightManager.module("Entities", function(Entities,
		...
		Entities.Graph = Backbone.Model.extend({
			...
			defaults : {
				...
				y : [],// 组件y轴属性
				mapping : {}
				,// 组件的映射字段，不同图表类型所需的映射字段列表不同
				/*
				 * shap: '',//组件的形状映射属性 color: 'white',//组件的颜色映射属性 size:
				 * 1,//组件大小映射属性 fill: ''//组件填充字段映射属性
				 */
			},
			/**
			...
			initialize : function() {
				...
				// 映射关系
				this.map = {
					"area" : ["alpha", "colour", "fill", "linetype", "size"],
					"bar" : ["alpha", "colour", "fill", "linetype", "size",
							"weight"],
					"pie" : ["alpha", "colour", "linetype", "size"],
					"boxplot" : ["alpha", "colour", "fill", "linetype",
							"shape", "size", "weight"],
					"line" : ["alpha", "colour", "linetype", "size"],
					"map" : ["alpha", "colour", "fill", "linetype", "size",
							"map_id"],
					"path" : ["alpha", "colour", "linetype", "size"],
					"point" : ["alpha", "colour", "fill", "size", "shape"],
					"polygon" : ["alpha", "colour", "fill", "linetype", "size",
							"group"],
					"ribbon" : ["alpha", "colour", "fill", "linetype", "size"],
					"tile" : ["alpha", "colour", "fill", "linetype", "size"],
					"violin" : ["alpha", "colour", "fill", "linetype", "size",
							"weight"]
				};
			 ...
			 */
			getMaping : function() {
				var self = this, featureArr = eval("this.map." + this.graph);
				// 默认的映射
				if (!featureArr) {
			...
			listenChange : function() {
				 ...
				 * self.on("change", function(){ //通知入口model图表发生改变
				 * Entities.trigger("graph:change", this.toJSON()); //通知视图重画
				 * self.trigger("graph:change"); }, self);
				 * self.trigger("change"); }); window.newArea =
		...
		/**
		 * 对外暴露接口 getGraphEntity:获取图表实体，如果是编辑的情况下则会去后台抓取。新建的情况下只是新建一个图表实体（没值）
		 */
		var API = {
			getGraphEntity : function() {
				var defer = $.Deferred()
				var id = window.widgetId;// TODO 这里获取后台传过来的id，如果是新建则为空
				// window.newArea为true的时候说明是新建工作区，则不会到后台请求数据,这个值在switch的controller触发switch:new事件
				// 的时候赋值为true
