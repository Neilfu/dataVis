...
(function(global, factory) {
	...
	(function(window) {
		...
		runescape = new RegExp("\\\\([\\da-f]{1,6}" + whitespace + "?|("
						+ whitespace + ")|.)", "ig"), funescape = function(_,
			...
			sortOrder = hasCompare ? function(a, b) {
				...
				if (!aup || !bup) {
					return a === doc ? -1 : b === doc ? 1 : aup ? -1 : bup
							? 1
							: sortInput ? (indexOf.call(sortInput, a) - indexOf
									.call(sortInput, b)) : 0;
		...
		Sizzle.attr = function(elem, name) {
			...
			return val !== undefined ? val : support.attributes
					|| !documentIsHTML ? elem.getAttribute(name) : (val = elem
		...
		function setMatcher(preFilter, selector, matcher, postFilter,
			...
			return markFunction(function(seed, results, context, xml) {
				...
				elems = seed
						|| multipleContexts(selector || "*", context.nodeType
										? [context]
										: context, []),
				...
				for (; i < length; i++) {
					if (resolveValues[i]
						...
						resolveValues[i].promise().done(updateFunc(i,
								...
								.fail(deferred.reject).progress(updateFunc(i,
	...
	function dataAttr(elem, key, data) {
		...
		if (data === undefined && elem.nodeType === 1) {
			...
			if (typeof data === "string") {
				try {
					data = data === "true" ? true : data === "false"
							? false
							: data === "null" ? null :
									// Only convert to a number if it doesn't
									// change the string
									+data + "" === data ? +data : rbrace
											.test(data) ? jQuery
											.parseJSON(data) : data;
				} catch (e) {
				}
				...
				data : function(key, value) {
					...
					return access(this, function(value) {
								...
								this.each(function() {
											...
											// reference of any
											// data that might've been store
											// with a camelCased key.
											var data = data_user.get(this,
													camelKey);

											// For HTML5 data-* attribute
											// interop, we have to
											// store property names with dashes
											// in a camelCase form.
											// This might not apply to all
											// properties...*
											data_user
													.set(this, camelKey, value);

											// *... In the case of properties
											// that might _actually_
											// have dashes, we need to also
											// store a copy of that
											// unchanged property.
											if (key.indexOf("-") !== -1
													&& data !== undefined) {
...
(function() {
	...
	jQuery.event = {
		...
		trigger : function(event, data, elem, onlyHandlers) {
			...
			var i, cur, tmp, bubbleType, ontype, handle, special, eventPath = [elem
					...
					? event.type
					: event, namespaces = hasOwn.call(event, "namespace")
	...
	jQuery.Event = function(src, props) {
		...
		if (src && src.type) {
			...
			this.isDefaultPrevented = src.defaultPrevented
					||
					// Support: Android < 4.0
					src.defaultPrevented === undefined && src.getPreventDefault
	...
	function getAll(context, tag) {
		var ret = context.getElementsByTagName ? context
				.getElementsByTagName(tag || "*") : context.querySelectorAll
	...
	jQuery.fn.extend({
		...
		domManip : function(args, callback) {
			...
			var fragment, first, scripts, hasScripts, node, doc, i = 0, l = this.length, set = this, iNoClone = l
	...
	jQuery.each({
				...
				jQuery.fn[name] = function(selector) {
					var elems, ret = [], insert = jQuery(selector), last = insert.length
							- 1, i = 0;
	...
	function actualDisplay(name, doc) {
		...
		display = window.getDefaultComputedStyle ?
				...
				// since it was removed from specification and supported only in
				// FF
		...
		return {
			get : function() {
				if (conditionFn()) {
					...
					// missing dependency),
					// remove it.
					// Since there are no other hooks for marginRight, remove
					// the whole object.
					delete this.get;
					return;
				}
...
(function() {
		...
		if (window.getComputedStyle) {
			jQuery.extend(support, {
				pixelPosition : function() {
					...
					// No need to check if the test was already performed,
					// though.
				...
				reliableMarginRight : function() {
					...
					// for margin-right
					// This support function is only executed once so no
					// memoizing is needed.
	...
	jQuery.extend({
		...
		style : function(elem, name, value, extra) {
			...
			if (value !== undefined) {
				...
				// Fixes #8908, it can be done more correctly by specifying
				// setters in cssHooks,
				...
				if (!hooks
					...
					// Setting style to blank string required to delete "style:
					// x !important;"
					style[name] = "";
					style[name] = value;
				}
				...
				jQuery.cssHooks[name] = {
					get : function(elem, computed, extra) {
						if (computed) {
							...
							// invisibly show them
							// however, it must have a current display style
							// that would benefit from this
							return elem.offsetWidth === 0
									&& rdisplayswap.test(jQuery.css(elem,
											"display")) ? jQuery.swap(elem,
									cssShow, function() {
		...
		finish : function(type) {
			...
			return this.each(function() {
				var index, data = data_priv.get(this), queue = data[type
						+ "queue"], hooks = data[type + "queueHooks"], timers = jQuery.timers, length = queue
						? queue.length
						: 0;
	...
	jQuery.each(["toggle", "show", "hide"], function(i, name) {
				...
				jQuery.fn[name] = function(speed, easing, callback) {
					return speed == null || typeof speed === "boolean" ? cssFn
							.apply(this, arguments) : this.animate(genFx(name,
...
(function() {
			...
			select : {
				get : function(elem) {
					var value, option, options = elem.options, index = elem.selectedIndex, one = elem.type === "select-one"
							|| index < 0, values = one ? null : [], max = one
							? index + 1
							: options.length, i = index < 0 ? max : one
							? index
							: 0;
							...
							if (callback) {
								...
								if (type === "abort") {
									...
									complete(xhrSuccessStatus[xhr.status]
											...
											// responseText throws an exception
											// (#11426)
	...
	jQuery.fn.extend({
		...
		position : function() {
			...
			// left: 0}, because it is its only offset parent
								...
								return access(
										...
										function(elem, type, value) {
											...
											if (jQuery.isWindow(elem)) {
												// As of 5/8/2012 this will
												// yield incorrect results for
												// Mobile Safari, but there
												// isn't a whole lot we can do.
												// See pull request at this URL
												// for discussion:
											...
											return value === undefined ?
													// Get width or height on
													// the element, requesting
													// but not forcing
													// parseFloat
