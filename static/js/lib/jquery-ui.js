/*
 * ! jQuery UI - v1.11.0 - 2014-06-26 http://jqueryui.com Includes: core.js,
 * widget.js, mouse.js, position.js, draggable.js, droppable.js, resizable.js,
 * selectable.js, sortable.js, accordion.js, autocomplete.js, button.js,
 * datepicker.js, dialog.js, menu.js, progressbar.js, selectmenu.js, slider.js,
 * spinner.js, tabs.js, tooltip.js, effect.js, effect-blind.js,
 * effect-bounce.js, effect-clip.js, effect-drop.js, effect-explode.js,
 * effect-fade.js, effect-fold.js, effect-highlight.js, effect-puff.js,
 * effect-pulsate.js, effect-scale.js, effect-shake.js, effect-size.js,
 * effect-slide.js, effect-transfer.js Copyright 2014 jQuery Foundation and
	...
	$.fn.extend({
				...
				disableSelection : (function() {
					var eventType = "onselectstart" in document
							.createElement("div") ? "selectstart" : "mousedown";
		...
		$.fn.position = function(options) {
			...
			return this.each(function() {
				var collisionPosition, using, elem = $(this), elemWidth = elem
						...
						basePosition), myOffset = getOffsets(offsets.my, elem
...
// fraction support test
	...
	$.widget("ui.draggable", $.ui.mouse, {
		...
		_removeHandleClassName : function() {
			this.element.find(".ui-draggable-handle").addBack()
	...
	$.widget("ui.resizable", $.ui.mouse, {
				...
				for (i in this.handles) {
					...
					if (this.handles[i].constructor === String) {
						this.handles[i] = this.element
								.children(this.handles[i]).first().show();
					}
		...
		_mouseDrag : function(event) {
			...
			var data, el = this.helper, props = {}, smp = this.originalMousePosition, a = this.axis, dx = (event.pageX - smp.left)
		...
		_respectSize : function(data) {
			...
			var o = this._vBoundaries, a = this.axis, ismaxw = this
					...
					&& o.maxWidth && (o.maxWidth < data.width), ismaxh = this
					._isNumber(data.height)
					&& o.maxHeight && (o.maxHeight < data.height), isminw = this
					._isNumber(data.width)
					&& o.minWidth && (o.minWidth > data.width), isminh = this
					._isNumber(data.height)
	...
	$.ui.plugin.add("resizable", "animate", {
		...
		stop : function(event) {
			var that = $(this).resizable("instance"), o = that.options, pr = that._proportionallyResizeElements, ista = pr.length
					...
					&& that._hasScroll(pr[0], "left") /* TODO - jump height */
					? 0
	...
	var sortable = $.widget("ui.sortable", $.ui.mouse, {
		...
		_setHandleClassName : function() {
			...
			$.each(this.items, function() {
						(this.instance.options.handle ? this.item
								.find(this.instance.options.handle) : this.item)
		...
		_mouseDrag : function(event) {
			...
			for (i = this.items.length - 1; i >= 0; i--) {
				...
				// the placeholder to jitter between the outer and inner
				// container.
		...
		_intersectsWithPointer : function(item) {
			...
			var isOverElementHeight = (this.options.axis === "x")
					|| this._isOverAxis(this.positionAbs.top
		...
		_intersectsWithSides : function(item) {
			...
			var isOverBottomHalf = this._isOverAxis(this.positionAbs.top
							+ this.offset.click.top, item.top
							+ (item.height / 2), item.height), isOverRightHalf = this
					._isOverAxis(
	...
	var accordion = $.widget("ui.accordion", {
		...
		_destroy : function() {
			...
			this.headers
					...
					.removeAttr("aria-selected").removeAttr("aria-controls")
			...
			contents = this.headers
					...
					.css("display", "").removeAttr("role")
					.removeAttr("aria-hidden").removeAttr("aria-labelledby")
					.removeUniqueId();
	...
	var menu = $.widget("ui.menu", {
		...
		_destroy : function() {
			...
			this.element.find(".ui-menu-item").removeClass("ui-menu-item")
					...
					.removeUniqueId().removeClass("ui-state-hover")
		...
		_keydown : function(event) {
			...
			switch (event.keyCode) {
				...
				default :
					...
					match = this.activeMenu.find(this.options.items).filter(
							function() {
		...
		refresh : function() {
			...
			items.not(".ui-menu-item, .ui-menu-divider")
					.addClass("ui-menu-item").uniqueId().attr({
								tabIndex : -1,
								role : this._itemRole()
							});
		...
		_close : function(startMenu) {
			...
			startMenu.find(".ui-menu").hide().attr("aria-hidden", "true").attr(
					"aria-expanded", "false").end().find(".ui-state-active")
					.not(".ui-state-focus").removeClass("ui-state-active");
		},
		...
		nextPage : function(event) {
			...
			if (this._hasScroll()) {
				...
				this.focus(event,
						this.activeMenu.find(this.options.items)[!this.active
								? "first"
								: "last"]());
			}
		},
	 ...
	 * datepicker_instActive, set by _updateDatepicker allows the handlers to
	 * find their way back to the active picker.
	 */
		...
		return dpDiv.delegate(selector, "mouseout", function() {
			...
			if (!$.datepicker
					._isDisabledDatepicker(datepicker_instActive.inline ? dpDiv
							.parent()[0] : datepicker_instActive.input[0])) {
	...
	var selectmenu = $.widget("ui.selectmenu", {
		...
		_drawMenu : function() {
			...
			this.menuInstance = this.menu.menu({
				...
				focus : function(event, ui) {
					...
					that.button.attr("aria-activedescendant", that.menuItems
									.eq(item.index).attr("id"));
				}
			}).menu("instance");
		...
		_setOption : function(key, value) {
			...
			if (key === "disabled") {
				...
				this.button.toggleClass("ui-state-disabled", value).attr(
						"aria-disabled", value);
		...
		_toggleAttr : function() {
			this.button.toggleClass("ui-corner-top", this.isOpen).toggleClass(
					"ui-corner-all", !this.isOpen).attr("aria-expanded",
					this.isOpen);
	...
	var slider = $.widget("ui.slider", $.ui.mouse, {
		...
		_createHandles : function() {
			var i, handleCount, options = this.options, existingHandles = this.element
					...
					.addClass("ui-state-default ui-corner-all"), handle = "<span class='ui-slider-handle ui-state-default ui-corner-all' tabindex='0'></span>", handles = [];
...
/** *************************************************************************** */
	...
	var effectBlind = $.effects.effect.blind = function(o, done) {
		...
		var el = $(this), rvertical = /up|down|vertical/, rpositivemotion = /up|left|vertical|horizontal/, props = [
				"position", "top", "bottom", "left", "right", "height", "width"], mode = $.effects
				.setMode(el, o.mode || "hide"), direction = o.direction || "up", vertical = rvertical
