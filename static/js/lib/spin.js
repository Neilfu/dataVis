...
(function(root, factory) {
	...
	function addAnimation(alpha, trail, i, lines) {
		...
		if (!animations[name]) {
			sheet.insertRule('@' + pre + 'keyframes ' + name + '{'
							+ '0%{opacity:' + z + '}' + start + '%{opacity:'
							+ alpha + '}' + (start + 0.01) + '%{opacity:1}'
	...
	merge(Spinner.prototype, {
		...
		spin : function(target) {
			...
			var self = this, o = self.opts, el = self.el = css(createEl(0, {
						...
						width : 0,
						zIndex : o.zIndex
					}), mid = o.radius + o.length + o.width, ep // element
																// position
			, tp // target position
			...
			if (target) {
				...
				ep = pos(el)
				css(el, {
							left : (o.left == 'auto' ? tp.x - ep.x
									+ (target.offsetWidth >> 1) : parseInt(
									o.left, 10)
									+ mid)
									+ 'px',
							top : (o.top == 'auto' ? tp.y - ep.y
									+ (target.offsetHeight >> 1) : parseInt(
									o.top, 10)
									+ mid)
									+ 'px'
						})
			...
			if (!useCssAnimations) {
				...
				var i = 0, start = (o.lines - 1) * (1 - o.direction) / 2, alpha, fps = o.fps, f = fps
	...
	(function anim() {
					...
					for (var j = 0; j < o.lines; j++) {
						alpha = Math.max(1 - (i + (o.lines - j) * astep) % f
										* ostep, o.opacity)
			...
			for (; i < o.lines; i++) {
				seg = css(createEl(), {
							...
							animation : useCssAnimations
									&& addAnimation(o.opacity, o.trail, start
													+ i * o.direction, o.lines)
