...
define(function(require) {
	...
	self.init = function(dom, theme) {
		...
		if (((zrender.version || '1.0.3').replace('.', '') - 0) < (self.dependencies.zrender
				.replace('.', '') - 0)) {
			console.error('ZRender ' + (zrender.version || '1.0.3-')
	 ...
	 * @type {Array}
	 */
	var ZR_EVENT_LISTENS = ['CLICK', 'MOUSEOVER', 'DRAGSTART', 'DRAGEND',
	 ...
	 *            arg2 调用参数3
	 */
	function callChartListMethodReverse(ecInstance, methodName, arg0, arg1,
			arg2) {
	...
	Echarts.prototype = {
		...
		_init : function() {
			...
			this._island = new Island(this._themeConfig, this._messageCenter,
					_zr, {}, this);
			...
			this._toolbox = new Toolbox(this._themeConfig, this._messageCenter,
					_zr, {}, this);
					...
					if (!fromMyself) {
						...
						if (grid) {
							this._zr.trigger('mousemove', {
										connectTrigger : true,
										zrenderX : grid.getX() + param.x
												* grid.getWidth(),
										zrenderY : grid.getY() + param.y
												* grid.getHeight()
									});
						}
						...
						var grid = this.component.grid;
						if (grid) {
							param.x = (param.event.zrenderX - grid.getX())
									/ grid.getWidth();
							param.y = (param.event.zrenderY - grid.getY())
									/ grid.getHeight();
						}
					}
					break;
				/*
				 * case ecConfig.EVENT.RESIZE : case ecConfig.EVENT.DATA_CHANGED :
				 * case ecConfig.EVENT.PIE_SELECTED : case
				 * ecConfig.EVENT.MAP_SELECTED : break;
				 */
			}
		...
		_onclick : function(param) {
			...
			if (param.target) {
				...
				if (ecData && ecData.seriesIndex != null) {
					this._messageCenter.dispatch(ecConfig.EVENT.CLICK,
							param.event, ecData, this);
				}
			}
		},
		...
		_onmouseover : function(param) {
			if (param.target) {
				var ecData = this._eventPackage(param.target);
				if (ecData && ecData.seriesIndex != null) {
					this._messageCenter.dispatch(ecConfig.EVENT.HOVER,
							param.event, ecData, this);
				}
			}
		},
		...
		_ondragend : function(param) {
			...
			if (this._status.needRefresh) {
				...
				messageCenter.dispatch(ecConfig.EVENT.DATA_CHANGED,
						param.event, this._eventPackage(param.target), this);
				messageCenter
						.dispatch(ecConfig.EVENT.REFRESH, null, null, this);
			}
		},
		...
		_onlegendSelected : function(param) {
			...
			callChartListMethodReverse(this, 'onlegendSelected', param,
					this._status);

			if (this._status.needRefresh) {
				this._messageCenter.dispatch(ecConfig.EVENT.REFRESH, null,
						null, this);
			}
		},
		...
		_ondataViewChanged : function(param) {
			...
			this._messageCenter.dispatch(ecConfig.EVENT.DATA_CHANGED, null,
					param, this);
			this._messageCenter.dispatch(ecConfig.EVENT.REFRESH, null, null,
					this);
		},
		...
		_syncBackupData : function(curOption) {
			this.component.dataZoom
					&& this.component.dataZoom.syncBackupData(curOption);
		},
		...
		_eventPackage : function(target) {
			if (target) {
				...
				dataIndex = seriesIndex != -1 && this.component.dataZoom
						? this.component.dataZoom.getRealDataIndex(seriesIndex,
								dataIndex)
						: dataIndex;
				return {
		...
		_render : function(magicOption) {
			...
			if (bgColor) {
				if (!_canvasSupported && bgColor.indexOf('rgba') != -1) {
					...
					this.dom.style.filter = 'alpha(opacity='
							+ cList[3].substring(0, cList[3].lastIndexOf(')'))
							* 100 + ')';
					cList.length = 3;
			...
			var componentList = ['title', 'legend', 'tooltip', 'dataRange',
			...
			for (var i = 0, l = componentList.length; i < l; i++) {
				...
				if (magicOption[componentType]) {
					if (component) {
						...
						ComponentClass = componentLibrary.get(/^[xy]Axis$/
										.test(componentType)
										? 'axis'
										: componentType);
						component = new ComponentClass(this._themeConfig,
								this._messageCenter, this._zr, magicOption,
								this, componentType);
				...
				if (!chartMap[chartType]) {
					...
					if (ChartClass) {
						if (this.chart[chartType]) {
							...
							chart = new ChartClass(this._themeConfig,
									this._messageCenter, this._zr, magicOption,
									this);
						}
			...
			for (chartType in this.chart) {
				if (chartType != ecConfig.CHART_TYPE_ISLAND
						&& !chartMap[chartType]) {
			...
			magicOption.animation && !magicOption.renderAsImage ? this._zr
					.refresh() : this._zr.render();
		 ...
		 * 
		 * @param {Object=}
		 *            param，可选参数，用于附带option，内部同步用，外部不建议带入数据修改，无法同步
		 */
		refresh : function(param) {
			...
			for (var i = 0, l = this._chartList.length; i < l; i++) {
				this._chartList[i].refresh
						&& this._chartList[i].refresh(magicOption);
			}
			this.component.grid
					&& this.component.grid.refixAxisShape(this.component);
			this._zr.refresh();
			...
			while (len--) {
				...
				if (chart) {
					...
					this.component[chartType]
							&& delete this.component[chartType];
		 ...
		 *            option
		 * @param {boolean=}
		 *            notMerge 多次调用时option选项是默认是合并（merge）的，
		...
		_setOption : function(option, notMerge) {
			if (!notMerge && this._option) {
				this._option = zrUtil.merge(this.getOption(), zrUtil
								.clone(option), true);
			} else {
			...
			if (this.component.dataZoom // 存在dataZoom控件
					...
					|| (this._option.toolbox && this._option.toolbox.feature
							&& this._option.toolbox.feature.dataZoom && this._option.toolbox.feature.dataZoom.show))) {
				// dataZoom同步数据
			...
			function restoreOption(prop) {
				...
				if (restoreSource) {
					if (restoreSource instanceof Array) {
						...
						while (len--) {
							magicOption[prop][len].data = zrUtil
									.clone(	restoreSource[len].data);
						}
					} else {
						magicOption[prop].data = zrUtil
		...
		_setTimelineOption : function(option) {
			...
			var timeline = new Timeline(this._themeConfig, this._messageCenter,
					this._zr, option, this);
		...
		/**
		 * 动态数据添加 形参为单组数据参数，多组时为数据，内容同[seriesIdx, data, isShift, additionData]
		 * 
		 * @param {number}
		 *            seriesIdx 系列索引
		 * @param {number |
		...
		addData : function(seriesIdx, data, isHead, dataGrow, additionData) {
			var params = seriesIdx instanceof Array ? seriesIdx : [[seriesIdx,
					data, isHead, dataGrow, additionData]];
				...
				if (seriesItem) {
					...
					if (additionData != null) {
						...
						if (seriesItem.type == ecConfig.CHART_TYPE_PIE
							...
							if (!dataGrow) {
								var legendDataIdx = zrUtil.indexOf(legendData,
										data.name);
								legendDataIdx != -1
										&& legendData.splice(legendDataIdx, 1);

								legendDataIdx = zrUtil.indexOf(mLegendData,
										data.name);
								legendDataIdx != -1
										&& mLegendData.splice(legendDataIdx, 1);
							}
						} else if (optionRestore.xAxis != null
								&& optionRestore.yAxis != null) {
							// x轴类目
							var axisData;
							var mAxisData;
			...
			// dataZoom同步数据
			this.component.dataZoom
					&& this.component.dataZoom.syncOption(magicOption);
			...
			setTimeout(function() {
						...
						for (var i = 0, l = chartList.length; i < l; i++) {
							// 有addData动画就去掉过渡动画
							chartList[i].motionlessOnce = magicOption.addDataAnimation
									&& chartList[i].addDataAnimation;
						}
						self._messageCenter.dispatch(ecConfig.EVENT.REFRESH,
								null, {
									option : magicOption
								}, self);
		...
		_delMark : function(seriesIdx, markName, markType) {
			...
			if (!(series && (seriesItem = series[seriesIdx])
					&& (mark = seriesItem[markType]) && (dataArray = mark.data))) {
				return this;
			}
			...
			if (targetIndex > -1) {
				...
				this._optionRestore.series[seriesIdx][markType].data.splice(
						targetIndex, 1);

				var chart = this.chart[seriesItem.type];
				chart
						&& chart.delMark(seriesIdx, markName.join(' > '),
								markType);
			}
		 ...
		 * 
		 * @param {string}
		 *            imgType 图片类型，支持png|jpeg，默认为png
		 * @return imgDataURL
		 */
		 ...
		 * 多图联动
		 * 
		 * @param connectTarget{ECharts |
		 *            Array <ECharts>} connectTarget 联动目标
		 */
		 ...
		 * 
		 * @param connectTarget{ECharts |
		 *            Array <ECharts>} connectTarget 解除联动目标
		 */
		...
		showLoading : function(loadingOption) {
			...
			var finalTextStyle = zrUtil.merge(zrUtil.clone(textStyle),
					this._themeConfig.textStyle);
			textStyle.textFont = finalTextStyle.fontStyle + ' '
					+ finalTextStyle.fontWeight + ' ' + finalTextStyle.fontSize
					+ 'px ' + finalTextStyle.fontFamily;
		...
		resize : function() {
			...
			return function() {
				...
				self.component.grid
						&& self.component.grid.refixAxisShape(self.component);
				self._zr.refresh();
				self._messageCenter.dispatch(ecConfig.EVENT.RESIZE, null, null,
						self);
				return self;
			};
		},
