...
define(function(require) {
			...
			function pointList(zr, oldShape, newShape, duration, easing) {
				...
				if (!oldShape) { // add
					...
					if (newShape.type == 'half-smooth-polygon') {
						oldPointList[newPointListLen - 1] = zrUtil
								.clone(newPointList[newPointListLen - 1]);
						oldPointList[newPointListLen - 2] = zrUtil
								.clone(newPointList[newPointListLen - 2]);
					}
					oldShape = {
						style : {
				...
				if (oldPointListLen == newPointListLen) {
					...
					// 原来短，新的长，补全
					newShape.style.pointList = oldPointList.concat(newPointList
							.slice(oldPointListLen));
				} else {
					// 原来长，新的短，截断
					newShape.style.pointList = oldPointList.slice(0,
							newPointListLen);
				}
			...
			function polygon(zr, oldShape, newShape, duration, easing) {
				var rect = require('zrender/shape/Polygon').prototype
						.getRect(newShape.style);
			...
			function chord(zr, oldShape, newShape, duration, easing) {
				...
				if (oldShape.style) {
					cloneStyle(newShape, oldShape, 'source0', 'source1',
							'target0', 'target1');
				}
			...
			function line(zr, oldShape, newShape, duration, easing) {
				...
				cloneStyle(newShape, oldShape, 'xStart', 'xEnd', 'yStart',
						'yEnd');
