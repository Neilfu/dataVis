...
define(function(require) {
	...
	function point(zr, effectList, shape, zlevel) {
		...
		var color = effect.color || shape.style.strokeColor
				|| shape.style.color;
		...
		var effectShape = new IconShape({
			...
			style : {
				brushType : 'stroke',
				iconType : (shape.style.iconType != 'pin' && shape.style.iconType != 'droplet')
		...
		if (canvasSupported) { // 提高性能，换成image
			effectShape.style.image = zr.shapeToImage(effectShape,
					effectShape.style.width + shadowBlur * 2 + 2,
					effectShape.style.height + shadowBlur * 2 + 2).style.image;
		...
		var offset = (effectShape.style.width / devicePixelRatio - shape.style.width)
				/ 2;
		...
		zr.animate(effectShape.id, '', true).when(duration, {
	...
	function largePoint(zr, effectList, shape, zlevel) {
		...
		for (var i = 0; i < 20; i++) {
			...
			zr.animate(effectShape.id, 'style', true).when(duration, clip1)
					.when(duration * 2, clip2).when(duration * 3, clip1).when(
							duration * 4, clip1).delay(Math.random() * duration
							* i)
	...
	function line(zr, effectList, shape, zlevel) {
		...
		var color = effect.color || shape.style.strokeColor
				|| shape.style.color;
		var shadowColor = effect.shadowColor || shape.style.strokeColor
				|| color;
		...
		if (canvasSupported) { // 提高性能，换成image
			effectShape.style.image = zr.shapeToImage(effectShape,
					(size + shadowBlur) * 2, (size + shadowBlur) * 2).style.image;
			effectShape = new ImageShape({
						zlevel : effectShape.zlevel,
						style : effectShape.style,
						draggable : false,
		...
		var distance = (shape.style.xStart - shape.style.xEnd)
				...
				* (shape.style.yStart - shape.style.yEnd);
		var duration = Math.round(Math.sqrt(Math.round(distance * effect.period
						* effect.period)));
		if (!shape.style.smooth) {
			// 直线
			zr.animate(effectShape.id, 'style', true).when(duration, {
						x : shape._x - offset,
						y : shape._y - offset
					}).start();
		} else {
			...
			var pointList = shape.style.pointList
					|| shape.getPointList(shape.style);
