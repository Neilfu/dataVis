	...
	Chain.prototype = {
		...
		buildLinePath : function(ctx, style) {
			...
			if (!style.lineType || style.lineType == 'solid') {
				...
				dashedLineTo(ctx, x, y + height / 2, x + width, y + height / 2,
						dashLength);
			}
		},
		...
		brushSymbol : function(ctx, style) {
			...
			for (var idx = 0, l = chainPoint.length; idx < l; idx++) {
				...
				if (curPoint.symbol != 'none') {
					...
					ctx.fillStyle = curPoint.isEmpty
							? '#fff'
							: style.strokeColor;
					ctx.closePath();
					ctx.fill();
				...
				if (curPoint.showLabel) {
					...
					if (curPoint.rotation) {
						...
						ctx.fillText(curPoint.name, curPoint.textX,
								curPoint.textY);
						ctx.restore();
					} else {
						ctx.fillText(curPoint.name, curPoint.textX,
		...
		_updateTextTransform : function(ctx, rotation) {
			...
			if (rotation[0] !== 0) {
				...
				if (originX || originY) {
					matrix.translate(_transform, _transform, [-originX,
									-originY]);
				}
				matrix.rotate(_transform, _transform, rotation[0]);
				if (originX || originY) {
					matrix
							.translate(_transform, _transform, [originX,
											originY]);
				}
			}
