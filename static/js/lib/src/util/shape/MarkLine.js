	...
	MarkLine.prototype = {
		...
		brush : function(ctx, isHighlight) {
			...
			if (isHighlight) {
				// 根据style扩展默认高亮样式
				style = this
						.getHighlightStyle(style, this.highlightStyle || {});
			}
		...
		buildLinePath : function(ctx, style) {
			...
			var len = Math.min(style.pointList.length, Math
							.round(style.pointListLength
			...
			if (!style.lineType || style.lineType == 'solid') {
				...
				if (style.smooth !== 'spline') {
					...
					ctx.moveTo(pointList[0][0], pointList[0][1]);
					for (var i = 1; i < len; i++) {
						dashedLineTo(ctx, pointList[i - 1][0],
								pointList[i - 1][1], pointList[i][0],
								pointList[i][1], dashLength);
					}
				} else {
					// 曲线
		...
		brushSymbol : function(ctx, style, idx) {
			...
			var x = idx === 0
					? style.pointList[0][0]
					: style.pointList[len - 1][0];
			var y = idx === 0
					? style.pointList[0][1]
					: style.pointList[len - 1][1];
			var rotate = typeof style.symbolRotate[idx] != 'undefined'
					? (style.symbolRotate[idx] - 0)
					: 0;
			var transform;
			if (rotate !== 0) {
		...
		getPointList : function(style) {
			var pointList = [[style.xStart, style.yStart],
		...
		/**
		 * {Array} start point {Array} end point
		 */
		getOffetPoint : function(sp, ep) {
			var distance = Math.sqrt(Math.round((sp[0] - ep[0])
							* (sp[0] - ep[0]) + (sp[1] - ep[1])
							* (sp[1] - ep[1])))
					/ 3;
			// console.log(delta);
		...
		isCover : function(x, y) {
			...
			if (x >= rect.x && x <= (rect.x + rect.width) && y >= rect.y
				...
				// 矩形内
				return this.style.smooth !== 'spline' ? area.isInside(
						lineInstance, this.style, x, y) : area.isInside(
						brokenLineInstance, this.style, x, y);
			}
