 ...
 * 
 * shape类：icon 可配图形属性： { // 基础属性 shape : 'icon', // 必须，shape类标识，需要显式指定 id :
 * {string}, // 必须，图形唯一标识，可通过'zrender/tool/guid'方法生成 zlevel : {number}, //
 * 默认为0，z层level，决定绘画在哪层canvas中 invisible : {boolean}, // 默认为false，是否可见
 *  // 样式属性，默认状态样式样式属性 style : { x : {number}, // 必须，左上角横坐标 y : {number}, //
 * 必须，左上角纵坐标 width : {number}, // 必须，宽度 height : {number}, // 必须，高度 iconType :
 * {string}, // 必须，icon类型 },
...
define(function(require) {
			...
			function _iconRestore(ctx, style) {
				...
				ctx.arc(style.x + r, style.y + r, r - dx, Math.PI, Math.PI * 5
								/ 3);
