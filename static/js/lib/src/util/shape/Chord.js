 ...
 * 
 * shape类：chord 可配图形属性： { // 基础属性 shape : 'chord', // 必须，shape类标识，需要显式指定 id :
 * {string}, // 必须，图形唯一标识，可通过'zrender/tool/guid'方法生成 zlevel : {number}, //
 * 默认为0，z层level，决定绘画在哪层canvas中 invisible : {boolean}, // 默认为false，是否可见
 *  // 样式属性，默认状态样式样式属性 style : { center : {array}, source0 : {number}, source1 :
 * {number}, target0 : {number}, target1 : {number}, r : {number}, },
...
define(function(require) {
	...
	ChordShape.prototype = {
		...
		buildPath : function(ctx, style) {
			...
			ctx.bezierCurveTo((cx - sx1) * 0.70 + sx1, (cy - sy1) * 0.70 + sy1,
					(cx - tx0) * 0.70 + tx0, (cy - ty0) * 0.70 + ty0, tx0, ty0);
			// Chord to self
			...
			ctx.bezierCurveTo((cx - tx1) * 0.70 + tx1, (cy - ty1) * 0.70 + ty1,
					(cx - sx0) * 0.70 + sx0, (cy - sy0) * 0.70 + sy0, sx0, sy0);
		},
