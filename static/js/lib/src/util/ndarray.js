...
define(function(require) {
	...
	var NDArray = function(array) {
		...
		if (array && typeof(array) !== 'string') {
			...
			/**
			 * _array Initialized with an empty array Data is continuous
			 * one-dimensional array, row-major A [2, 2] dim empty array is
			 * stored like [0,0, 0,0] TODO : Consider column majors ?
			 * 
			 * @type {ArrayConstructor}
			 */
			this._array = new ArrayConstructor[this._dtype]();
			/**
			 * _shape a tuple array describe the dimension and size of each
			 * dimension [10, 10] means a 10x10 array
			 * 
			 * @type {Array}
			 */
			this._shape = [0];
			/**
			 * _size size of the storage array length
			 * 
			 * @type {Number}
			 */
			this._size = 0;
	...
	NDArray.prototype = {
		...
		/**
		 * Get array data type. 'int32' 'int16' 'int8' 'uint32' 'uint16' 'uint8'
		 * 'float32' 'float64'
		 * 
		 * @return {String}
		 */
		...
		_transposelike : function(axes, out) {
			...
			function transpose(axis, offset, transposedOffset) {
				...
				if (axis < dim - 1) {
					for (var i = 0; i < size; i++) {
						transpose(axis + 1, offset + stride * i,
								transposedOffset + transposedStride * i);
					}
				} else {
					for (var i = 0; i < size; i++) {
						// offset + stride * i is the index of the original
						// array
						// transposedOffset + i is the index of the transposed
						// array
						transposedData[transposedOffset + i] = source[offset
								+ stride * i];
					}
				}
			}
		 ...
		 * @param {Number}
		 *            [axis] The axis along which to repeat values. By default,
		 *            use the flattened input array, and return a flat output
		 *            array.
		 * @param {NDArray}
		 *            [out]
		 * @return {NDArray}
		...
		/**
		 * Preprocess for array calculation max, min, argmax, argmin, sum, ptp,
		 * val, mean Which will reduce one axis if the axis is given
		 * 
		 * @param {Number}
		 *            axis
		 * @param {NDArray}
		...
		_withPreprocess1 : function(axis, out, funcWithAxis, funcFlatten) {
			...
			if (typeof(axis) !== 'undefined') {
				...
				funcWithAxis.call(this, data, source, offsetStride, axisSize,
						stride);
		...
		/**
		 * Preprocess for array calculation cumsum, cumprod Which will keep the
		 * shape if axis is given and flatten if axis is undefined
		 * 
		 * @param {Number}
		 *            axis
		 * @param {NDArray}
		...
		/**
		 * Get the max value of ndarray If the axis is given, the max is only
		 * calculate in this dimension Example, for the given ndarray [[3, 9],
		 * [4, 8]] >>> max(0) [4, 9] >>> max(1) [9, 8]
		 * 
		 * @param {Number}
		 *            [axis]
		 * @param {NDArray}
		...
		/**
		 * Return the standard derivatione of the array elements over the given
		 * axis.
		 * 
		 * @param {Number}
		 *            [axis]
		...
		argsort : kwargs(function(axis, order, out) {
					...
					if (out && !arrayEqual(this._shape, out._shape)) {
						throw new Error(broadcastErrorMsg(this._shape,
								out._shape));
					}
					if (!out) {
						out = new NDArray(this._dtype);
		 ...
		 * 
		 * @param {string}
		 *            index Index syntax can be an integer 1, 2, 3 Or more
		 *            complex range indexing '1:2' '1:2, 1:2' '1:2, :' More
		 *            about the indexing syntax can check the doc of numpy
		 *            ndarray
		 * @param {NDArray}
		 *            [out]
	...
	NDArray.range = kwargs(function(min, max, step, dtype) {
				...
				var array = new ArrayConstructor[dtype](Math.ceil((max - min)
						/ step));
				var cursor = 0;
	...
	/**
	 * Python like array indexing http://www.python.org/dev/peps/pep-0204/
	 * 
	 * @param {string}
	 *            index index can be a simple integer 1,2,3, or a range 2:10,
	 *            2:10:1 example : 2:10 => [2, 10, 1], 10:2:-2 => [10, 2, -2], : =>
	 *            [0, dimSize, 1], ::-1 => [dimSize-1, -1, -1],
	 * @param {number}
	 *            dimSize
	...
	function broadcastErrorMsg(shape1, shape2) {
		return 'Shape (' + shape1.toString() + ') (' + shape2.toString()
