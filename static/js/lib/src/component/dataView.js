	...
	DataView.prototype = {
		...
		show : function(newOption) {
			...
			this._tDom.innerHTML = '<p style="padding:8px 0;margin:0 0 10px 0;'
					...
					+ (lang[0] || this._lang[0]) + '</p>';

			this._textArea.style.cssText = 'display:block;margin:0 0 8px 0;padding:4px 6px;overflow:auto;'
					+ 'width:'
					+ (this._zrWidth - 15)
					+ 'px;'
					+ 'height:'
					+ (this._zrHeight - 100) + 'px;';
			var customContent = this.query(this.option,
			...
			this._sizeCssText = 'width:' + this._zrWidth + 'px;' + 'height:'
					+ this._zrHeight + 'px;' + 'background-color:#fff;';
			this._tDom.style.cssText = this._gCssText + this._sizeCssText;
			// 这是个很恶心的事情
			/*
			 * this.dom.onselectstart = function () { return true; };
			 */
		},
		...
		_optionToContent : function() {
				...
				for (i = 0, len = axisList.length; i < len; i++) {
					...
					if ((axisList[i].type || 'category') == 'category') {
						...
						for (j = 0, k = axisList[i].data.length; j < k; j++) {
							...
							valueList.push(typeof data.value != 'undefined'
											? data.value
											: data);
						}
			...
			for (i = 0, len = series.length; i < len; i++) {
				...
				content += valueList
						.join(	series[i].type == ecConfig.CHART_TYPE_SCATTER
										? '\n'
										: ', ');
				content += '\n\n';
			}
