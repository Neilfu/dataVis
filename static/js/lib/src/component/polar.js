...
define(function(require) {
	...
	Polar.prototype = {
		...
		_createVector : function(index) {
			...
			var radius = this.parsePercent(item.radius, Math.min(this.zr
			...
			for (var i = 0; i < length; i++) {
				vector = ecCoordinates.polar2cartesian(radius, startAngle
								* Math.PI / 180 + dStep * i);
				__ecIndicator.push({
							// 将图形翻转
		...
		_buildSpiderWeb : function(index) {
			...
			this._addArea(__ecIndicator, splitNumber, center, splitArea,
					strokeColor, lineWidth, show);

			this._addLine(__ecIndicator, center, axisLine);
		},
		...
		_addAxisLabel : function(index) {
			...
			for (var i = 0; i < indicator.length; i++) {
				axisLabel = this.deepQuery([indicator[i], item, this.option],
						'axisLabel');
				...
				if (axisLabel.show) {
					...
					for (var j = 1; j <= splitNumber; j++) {
						...
						text = j * (value.max - value.min) / splitNumber
								+ value.min;
						if (precision) {
		...
		_buildText : function(index) {
			...
			for (var i = 0; i < indicator.length; i++) {
				name = this
						.deepQuery([indicator[i], item, this.option], 'name');

				if (!name.show) {
					continue;
				}
				textStyle = this.deepQuery([name, item, this.option],
						'textStyle');
				...
				if (typeof name.formatter == 'function') {
					...
					style.text = name.formatter.replace('{value}',
							indicator[i].text);
				} else {
				...
				if (name.rotate) {
					rotation = [name.rotate / 180 * Math.PI, vector[0],
							vector[1]];
				} else {
					rotation = [0, 0, 0];
				}
		 ...
		 *            线条宽度
		 */
		_addArea : function(__ecIndicator, splitNumber, center, splitArea,
				strokeColor, lineWidth, show) {
			var shape;
			var scale;
			var scale1;
			...
			for (var i = 0; i < splitNumber; i++) {
				...
				if (show) {
					shape = this._getShape(pointList, 'stroke', '',
				...
				if (splitArea.show) {
					...
					this._addSplitArea(__ecIndicator, splitArea, scale, scale1,
							center, i);
				}
			}
		},
		 ...
		 *            <Array<number>>} 绘制的点集
		 * @param {string}
		 *            绘制方式 stroke | fill | both 描边 | 填充 | 描边 + 填充
		 * @param {string}
		 *            颜色
		 * @param {string}
		 *            描边颜色
		 ...
		 */
		_getShape : function(pointList, brushType, color, strokeColor,
				lineWidth, hoverable, draggable) {
		 ...
		 * 绘制填充区域
		 */
		_addSplitArea : function(__ecIndicator, splitArea, scale, scale1,
				center, colorInd) {
		...
		_mapVector : function(vector, center, scale) {
			return [vector[0] * scale + center[0],
		 ...
		 *            线条宽度
		 * @param {string}
		 *            线条绘制类型 solid | dotted | dashed 实线 | 点线 | 虚线
		 */
		_addLine : function(__ecIndicator, center, axisLine) {
			...
			for (var i = 0; i < indLen; i++) {
				vector = __ecIndicator[i].vector;
				line = this._getLine(center[0], center[1], vector[0]
								+ center[0], vector[1] + center[1],
		 ...
		 * @return {Object} 线条对象
		 */
		_getLine : function(xStart, yStart, xEnd, yEnd, strokeColor, lineWidth,
				lineType) {
		...
		_adjustIndicatorValue : function(index) {
			...
			for (var i = 0; i < len; i++) {
				if (typeof indicator[i].max == 'number') {
					...
					value = this._findValue(data, i, splitNumber, boundaryGap,
		...
		_getSeriesData : function(index) {
			...
			for (var i = 0; i < this.series.length; i++) {
				...
				for (var j = 0; j < serieData.length; j++) {
					polarIndex = this.deepQuery([serieData[j], serie,
									this.option], 'polarIndex')
							|| 0;
					if (polarIndex == index
		 ...
		 * 查找指标合适的值
		 * 
		 * 如果只有一组数据以数据中的最大值作为最大值 0为最小值 如果是多组，使用同一维度的进行比较 选出最大值最小值 对它们进行处理
		 * 
		 * @param {Object}
		 *            serie 的 data
		 * @param {number}
		 ...
		 */
		_findValue : function(data, index, splitNumber, boundaryGap, precision,
				power, scale) {
			var max;
			var min;
			var value;
			...
			if (data.length != 1) {
				...
				for (var i = 0; i < one.value.length; i++) {
					_compare(typeof one.value[i].value != 'undefined'
									? one.value[i].value
									: one.value[i]);
				}
			}
			...
			if (data.length != 1) {
				if (scale) {
					delta = this._getDelta(max, min, splitNumber, precision,
							power);
					...
					if (delta >= 1) {
						...
						str = (delta + '').split('.')[1];
						len = str.length;
						min = Math.floor(min * Math.pow(10, len))
								/ Math.pow(10, len) - delta;
					}
					...
					max = min
							+ Math.floor(delta * Math.pow(10, len)
		...
		_getDelta : function(max, min, splitNumber, precision, power) {
			...
			if (delta > 1) {
				...
				if (!precision) {
					...
					if (str[n] >= 5) {
						return '0.' + str.substring(0, n + 1) - 0 + 1
								/ Math.pow(10, n);
					} else {
						return '0.' + str.substring(0, n + 1) - 0 + 1
								/ Math.pow(10, n + 1);
		 ...
		 * @param {Array
		 *            <number>} 坐标
		 * @return {Object} | false polarIndex valueIndex
		 */
		getNearestIndex : function(vector) {
			...
			for (var i = 0; i < this.polar.length; i++) {
				...
				angle = 2 * Math.PI / len;
				// 注意y轴的翻转
				polarVector = ecCoordinates.cartesian2polar(vector[0]
								- center[0], center[1] - vector[1]);
				...
				finalAngle = polarVector[1] - startAngle / 180 * Math.PI
						+ Math.PI * 2;

				if (Math.abs(Math.cos(finalAngle % (angle / 2))) * radius > polarVector[0]) {
					return {
						polarIndex : i,
						valueIndex : Math.floor((finalAngle + angle / 2)
										/ angle)
								% len
					};
				}
