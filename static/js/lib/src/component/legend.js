...
define(function(require) {
	...
	Legend.prototype = {
		...
		_buildItem : function() {
			...
			for (var i = 0; i < dataLength; i++) {
				dataTextStyle = zrUtil
						.merge(data[i].textStyle || {}, textStyle);
				...
				if (itemName === '') {
					if (this.legendOption.orient == 'horizontal') {
						...
						this.legendOption.x == 'right'
								? lastX -= this._itemGroupLocation.maxWidth
										+ itemGap
								: lastX += this._itemGroupLocation.maxWidth
										+ itemGap;
				...
				itemType = data[i].icon
						|| this._getSomethingByName(itemName).type;
				...
				// 图形
				itemShape = this._getItemShapeByType(lastX, lastY, itemWidth,
						itemHeight, (this._selectedMap[itemName]
								? color
								: '#ccc'), itemType, color);
				...
				if (this.legendOption.orient == 'horizontal') {
					lastX += itemWidth + 5
							+ zrArea.getTextWidth(itemName, dataFont) + itemGap;
				} else {
					lastY += itemHeight + itemGap;
				}
			}
							...
							.push(	(this._itemGroupLocation.width - (this.shapeList[i].style.x
											+ zrArea
													.getTextWidth(
															...
															this.shapeList[i].style.textFont) - lastX))
											/ 2);
				}
			}
			var curLineIndex = -1;
		...
		_getItemGroupLocation : function() {
			...
			if (this.legendOption.orient == 'horizontal') {
				...
				for (var i = 0; i < dataLength; i++) {
					...
					dataTextStyle = zrUtil.merge(data[i].textStyle || {},
							textStyle);
					temp += itemWidth
							+ zrArea.getTextWidth(this._getName(data[i]),
									data[i].textStyle ? this.getFont(zrUtil
											.merge(data[i].textStyle || {},
													textStyle)) : font)
							+ itemGap;
				}
				...
				// 垂直布局，计算总高度
				for (var i = 0; i < dataLength; i++) {
					maxWidth = Math.max(maxWidth, zrArea.getTextWidth(this
											._getName(data[i]),
									data[i].textStyle ? this.getFont(zrUtil
											.merge(data[i].textStyle || {},
													textStyle)) : font));
			...
			switch (this.legendOption.x) {
				case 'center' :
					...
					break;
				case 'left' :
					x = this.legendOption.padding[3]
							+ this.legendOption.borderWidth;
					break;
				case 'right' :
					x = zrWidth - totalWidth - this.legendOption.padding[1]
			...
			switch (this.legendOption.y) {
				case 'top' :
					y = this.legendOption.padding[0]
							+ this.legendOption.borderWidth;
					break;
				case 'bottom' :
					y = zrHeight - totalHeight - this.legendOption.padding[0]
		...
		_getItemShapeByType : function(x, y, width, height, color, itemType,
				defaultColor) {
			...
			switch (itemType) {
				...
				case 'k' :
					...
					itemShape.highlightStyle.lineWidth = 3;
					itemShape.highlightStyle.color = itemShape.style.color = this
							.query(this.ecTheme, 'k.itemStyle.normal.color')
							|| '#fff';
					itemShape.style.strokeColor = color != '#ccc'
							? (this.query(this.ecTheme,
									'k.itemStyle.normal.lineStyle.color') || '#ff3200')
							: color;
					break;
				case 'image' :
		...
		__legendSelected : function(param) {
			...
			this.messageCenter.dispatch(ecConfig.EVENT.LEGEND_SELECTED,
					param.event, {
		...
		refresh : function(newOption) {
			if (newOption) {
				...
				// 补全padding属性
				this.option.legend.padding = this
						.reformCssArray(this.option.legend.padding);
					...
					if (!something.series) {
						...
						if (something.data
								&& (something.type == ecConfig.CHART_TYPE_PIE
										|| something.type == ecConfig.CHART_TYPE_FORCE || something.type == ecConfig.CHART_TYPE_FUNNEL)) {
							queryTarget = [something.data, something.series];
						} else {
							queryTarget = [something.series];
						}

						color = this.getItemStyleColor(this.deepQuery(
										queryTarget, 'itemStyle.normal.color'),
								something.seriesIndex, something.dataIndex,
								something.data);
						if (color && something.type != ecConfig.CHART_TYPE_K) {
							this.setColor(itemName, color);
						}
						this._selectedMap[itemName] = typeof this._selectedMap[itemName] != 'undefined'
		...
		getColor : function(legendName) {
			if (!this._colorMap[legendName]) {
				this._colorMap[legendName] = this.zr
						.getColor(this._colorIndex++);
			}
	...
	var legendIcon = {
		...
		/*
		 * chord : function (ctx, style) { var x = style.x; var y = style.y; var
		 * width = style.width; var height = style.height; ctx.moveTo(x, y +
		 * height); BeziercurveShape.prototype.buildPath(ctx, { xStart : x,
		 * yStart : y + height, cpX1 : x + width, cpY1 : y + height, cpX2 : x,
		 * cpY2 : y + 4, xEnd : x + width, yEnd : y + 4 }); ctx.lineTo(x +
		 * width, y); BeziercurveShape.prototype.buildPath(ctx, { xStart : x +
		 * width, yStart : y, cpX1 : x, cpY1 : y, cpX2 : x + width, cpY2 : y +
		 * height - 4, xEnd : x, yEnd : y + height - 4 }); ctx.lineTo(x, y +
		 * height); },
		 */
		...
		bar : function(ctx, style) {
			...
			ctx.quadraticCurveTo(x + width, y + height, x + width - r, y
							+ height);
