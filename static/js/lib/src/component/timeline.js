...
define(function(require) {
	...
	function Timeline(ecTheme, messageCenter, zr, option, myChart) {
		...
		this.currentIndex = this.timelineOption.currentIndex
				% this.timelineOption.data.length;

		/*
		 * if (!this.timelineOption.notMerge) { for (var i = 1, l =
		 * this.timelineOption.data.length; i < l; i++) { this.options[i] =
		 * zrUtil.merge( this.options[i], this.options[i - 1] ); } }
		 */
	...
	Timeline.prototype = {
				...
				if (dataLength > 3) {
					...
					while (!isEnough && interval < dataLength) {
						...
						for (var i = interval; i < dataLength; i += interval) {
							labelSpace = chainPoint[i].x
									- chainPoint[i - interval].x;
							if (timelineOption.label.rotate !== 0) {
								...
								labelSize = wLen * fontSize * 2 / 3 + oLen
										* fontSize;
							}
		...
		_getChainPoint : function() {
			...
			if (len > 1) {
				...
				boundaryGap = boundaryGap > 50 ? 50 : (boundaryGap < 20
						? 5
						: boundaryGap);
				width -= boundaryGap * 2;
				if (timelineOption.type == 'number') {
					...
					xList[len - 1] = new Date(_getName(len - 1).replace(/-/g,
							'/'))
							- xList[0];
					for (var i = 1; i < len; i++) {
						xList[i] = x
			...
			for (var i = 0; i < len; i++) {
				x = xList[i];
				curSymbol = (data[i].symbol && data[i].symbol.toLowerCase())
						|| symbol;
				...
				dataTextStyle = data[i].textStyle ? zrUtil.merge(
						data[i].textStyle || {}, textStyle) : textStyle;
				...
				list.push({
							...
							textX : x,
							textY : y - (rotate ? 5 : 0),
							textFont : data[i].textStyle ? this
									.getFont(dataTextStyle) : textFont,
							rotation : rotation,
							showLabel : false
						});
			}
		...
		_buildChain : function() {
			...
			this._timelineShae = {
				...
				style : {
					x : this._location.x,
					y : this
							.subPixelOptimize(this._location.y, lineStyle.width),
		...
		_syncHandleShape : function() {
			...
			this._handleShape.style.text = cpStyle.label.show
					? curPoint.name
					: '';
			...
			if (cpStyle.symbol == 'auto') {
				...
				if (cpStyle.symbol.match('star')) {
					this._handleShape.style.n = (cpStyle.symbol.replace('star',
							'') - 0)
							|| 5;
			...
			this._handleShape.highlightStyle.strokeColor = this._handleShape.style.strokeColor = cpStyle.borderColor == 'auto'
			...
			this._handleShape.highlightStyle.lineWidth = this._handleShape.style.lineWidth
					+ 1;
		...
		_findChainIndex : function(x) {
			...
			for (var i = 0; i < len - 1; i++) {
				if (x >= chainPoint[i].x && x <= chainPoint[i + 1].x) {
					// catch you！
					return (Math.abs(x - chainPoint[i].x) < Math.abs(x
							- chainPoint[i + 1].x)) ? i : (i + 1);
				}
			}
		},
		...
		__ondrift : function(shape, dx) {
			...
			shape.style.y = this._location.y + this._location.height / 4
					- symbolSize;
		...
		setTheme : function(needRefresh) {
			this.timelineOption = this.reformOption(zrUtil
					.clone(this.option.timeline));
			// 补全padding属性
			this.timelineOption.padding = this
					.reformCssArray(this.timelineOption.padding);
			// 通用字体设置
