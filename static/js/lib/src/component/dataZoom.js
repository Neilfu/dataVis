	...
	DataZoom.prototype = {
		...
		_getZoom : function() {
			...
			var start = typeof this._zoom.start != 'undefined'
					? this._zoom.start
					: (typeof this.zoomOption.start != 'undefined'
							? this.zoomOption.start
							: 0);
			var end = typeof this._zoom.end != 'undefined'
					? this._zoom.end
					: (typeof this.zoomOption.end != 'undefined'
							? this.zoomOption.end
							: 100);
			/*
			 * var start = typeof this.zoomOption.start != 'undefined' &&
			 * this.zoomOption.start >= 0 && this.zoomOption.start <= 100 ?
			 * this.zoomOption.start : 0; var end = typeof this.zoomOption.end !=
			 * 'undefined' && this.zoomOption.end >= 0 && this.zoomOption.end <=
			 * 100 ? this.zoomOption.end : 100;
			 */
			if (start > end) {
				// 大小颠倒自动翻转
				start = start + end;
		...
		_calculScatterMap : function(seriesIndex) {
			...
			if (axisOption instanceof Array) {
				...
				axisOption[1]
						&& (axisOption[1].type = 'value', axisOption[1].boundary = [
								0, 0]);
			} else {
				axisOption.type = 'value';
				axisOption.boundary = [0, 0];
			}
			var vAxis = new Axis(this.ecTheme,
					null, // messageCenter
					false, // this.zr
					{
						xAxis : axisOption,
			...
			this._zoom.scatterMap[seriesIndex].x = vAxis.getAxis(axisIndex)
					.getExtremum();
			vAxis.dispose();
			...
			this._zoom.scatterMap[seriesIndex].y = vAxis.getAxis(axisIndex)
					.getExtremum();
			vAxis.dispose();
		...
		_buildBackground : function() {
			...
			if (this.zoomOption.orient == 'horizontal') {
				pointList.push([this._location.x + width,
		...
		_buildFiller : function() {
			...
			if (this.zoomOption.orient == 'horizontal') {
				...
				this._fillerShae.style = {
					x : this._location.x
							+ Math.round(this._zoom.start / 100
									* this._location.width) + this._handleSize,
					y : this._location.y,
				...
				this._fillerShae.style = {
					...
					y : this._location.y
							+ Math.round(this._zoom.start / 100
									* this._location.height) + this._handleSize,
		...
		_buildFrame : function() {
			...
			this._startFrameShape = {
				...
				style : {
					...
					y : y,
					width : this._location.width
							- (x > this._location.x ? 1 : 0),
					height : this._location.height
							- (y > this._location.y ? 1 : 0),
					lineWidth : 1,
					brushType : 'stroke',
		...
		_syncHandleShape : function() {
			if (this.zoomOption.orient == 'horizontal') {
				...
				this._zoom.start = Math
						.floor(	(this._startShape.style.x - this._location.x)
										/ this._location.width * 100);
				this._zoom.end = Math.ceil((this._endShape.style.x
				...
				this._endShape.style.y = this._fillerShae.style.y
						+ this._fillerShae.style.height;
				this._zoom.start = Math
						.floor(	(this._startShape.style.y - this._location.y)
										/ this._location.height * 100);
				this._zoom.end = Math.ceil((this._endShape.style.y
								+ this._handleSize - this._location.y)
								/ this._location.height * 100);
			}
		...
		_syncFillerShape : function() {
			...
			if (this.zoomOption.orient == 'horizontal') {
				...
				this._fillerShae.style.width = Math.abs(a - b)
						- this._handleSize;
				this._zoom.start = Math
						.floor(	(Math.min(a, b) - this._location.x)
										/ this._location.width * 100);
				this._zoom.end = Math
						.ceil(	(Math.max(a, b) + this._handleSize - this._location.x)
										/ this._location.width * 100);
			} else {
				a = this._startShape.style.y;
				b = this._endShape.style.y;
				this._fillerShae.style.y = Math.min(a, b) + this._handleSize;
				this._fillerShae.style.height = Math.abs(a - b)
						- this._handleSize;
				this._zoom.start = Math
						.floor(	(Math.min(a, b) - this._location.y)
										/ this._location.height * 100);
				this._zoom.end = Math
						.ceil(	(Math.max(a, b) + this._handleSize - this._location.y)
										/ this._location.height * 100);
			}
		...
		_syncFrameShape : function() {
			if (this.zoomOption.orient == 'horizontal') {
				this._startFrameShape.style.width = this._fillerShae.style.x
						- this._location.x;
				this._endFrameShape.style.x = this._fillerShae.style.x
						+ this._fillerShae.style.width;
				this._endFrameShape.style.width = this._location.x
						+ this._location.width - this._endFrameShape.style.x;
			} else {
				this._startFrameShape.style.height = this._fillerShae.style.y
						- this._location.y;
				this._endFrameShape.style.y = this._fillerShae.style.y
						+ this._fillerShae.style.height;
				this._endFrameShape.style.height = this._location.y
						+ this._location.height - this._endFrameShape.style.y;
			}
		...
		_syncShape : function() {
			...
			if (this.zoomOption.orient == 'horizontal') {
				this._startShape.style.x = this._location.x + this._zoom.start
						/ 100 * this._location.width;
				this._endShape.style.x = this._location.x + this._zoom.end
						/ 100 * this._location.width - this._handleSize;

				this._fillerShae.style.x = this._startShape.style.x
						+ this._handleSize;
				this._fillerShae.style.width = this._endShape.style.x
						- this._startShape.style.x - this._handleSize;
			} else {
				this._startShape.style.y = this._location.y + this._zoom.start
						/ 100 * this._location.height;
				this._endShape.style.y = this._location.y + this._zoom.end
						/ 100 * this._location.height - this._handleSize;

				this._fillerShae.style.y = this._startShape.style.y
						+ this._handleSize;
				this._fillerShae.style.height = this._endShape.style.y
						- this._startShape.style.y - this._handleSize;
			}
			...
			for (var key in this._originalData) {
				...
				for (var idx in target) {
					...
					if (this.option[key][idx].type != ecConfig.CHART_TYPE_SCATTER) {
						...
						// 散点图特殊处理
						this.option[key][idx].data = this._synScatterData(idx,
								data);
					}
				}
			}
		...
		_synScatterData : function(seriesIndex, data) {
			if (this._zoom.start === 0 && this._zoom.end == 100
					&& this._zoom.start2 === 0 && this._zoom.end2 == 100) {
				return data;
			}
			var newData = [];
			...
			for (var i = 0, l = data.length; i < l; i++) {
				...
				if (value[0] >= xStart && value[0] <= xEnd
						&& value[1] >= yStart && value[1] <= yEnd) {
					newData.push(data[i]);
				}
			}
		...
		__ondrift : function(shape, dx, dy) {
			...
			if (this.zoomOption.orient == 'horizontal') {
				...
				} else if (shape.style.x + dx + shape.style.width + detailSize >= this._location.x
						+ this._location.width) {
				...
				} else if (shape.style.y + dy + shape.style.height + detailSize >= this._location.y
						+ this._location.height) {
		...
		syncBackupData : function(curOption) {
			...
			for (var i = 0, l = curSeries.length; i < l; i++) {
				...
				if (target[i]) {
					// dataZoom接管的
					start = Math.floor(this._zoom.start / 100
							* target[i].length);
				} else {
					// 非dataZoom接管
					start = 0;
		...
		getRealDataIndex : function(sIdx, dIdx) {
			if (!this._originalData
					|| (this._zoom.start === 0 && this._zoom.end == 100)) {
				return dIdx;
			}
