...
define(function(require) {
	 ...
	 *            series 数据对象
	 */
	function ValueAxis(ecTheme, messageCenter, zr, option, myChart, axisBase,
			series) {
	...
	ValueAxis.prototype = {
		...
		_buildAxisTick : function() {
			...
			if (this.isHorizontal()) {
				...
				for (var i = 0; i < dataLength; i++) {
					// 亚像素优化
					x = this
							.subPixelOptimize(this.getCoord(data[i]), lineWidth);
					axShape = {
						_axisShape : 'axisTick',
				...
				for (var i = 0; i < dataLength; i++) {
					...
					if (rotate) {
						...
						axShape.rotation = [rotate * Math.PI / 180,
								axShape.style.x, axShape.style.y];
					}
					this.shapeList.push(new TextShape(this._axisLabelClickable(
									clickable, axShape)));
				}
			} else {
				// 纵向
			...
			if (isNaN(this.option.min - 0) || isNaN(this.option.max - 0)) {
				...
				for (var i = 0, l = this.series.length; i < l; i++) {
					...
					if (!this.series[i].stack) {
						...
						// 堆积数据，需要区分正负向堆积
						var keyP = '__Magic_Key_Positive__'
								+ this.series[i].stack;
						var keyN = '__Magic_Key_Negative__'
								+ this.series[i].stack;
						data[keyP] = data[keyP] || [];
						data[keyN] = data[keyN] || [];
		 ...
		 * 如果你有更简洁的数学推导欢迎重写，后果自负~ 一旦你不得不遇到了需要修改或重写的厄运，希望下面的脚手架能帮助你
		 * ps:其实我是想说别搞砸了！升级后至少得保证这些case通过！！
		 * 
		 * by linzhifeng@baidu.com 2013-1-8 -------- this._valueList = [];
		 * this.option = {splitNumber:5,power:100,precision:0}; this._min = 1;
		 * this._max = 123; console.log(this._min, this._max);
		 * this._reformValue(); console.log('result is :', this._min, this._max,
		 * this._valueList); console.log('should be : 0 150 [0, 30, 60, 90, 120,
		 * 150]', (this._min == 0 && this._max == 150) ? 'success' : 'failed');
		 * 
		 * this._min = 10; this._max = 1923; console.log(this._min, this._max);
		 * this._reformValue(); console.log('result is :', this._min, this._max,
		 * this._valueList); console.log('should be : 0 2000 [0, 400, 800, 1200,
		 * 1600, 2000]', (this._min == 0 && this._max == 2000) ? 'success' :
		 * 'failed');
		 * 
		 * this._min = 10; this._max = 78; console.log(this._min, this._max);
		 * this._reformValue(); console.log('result is :', this._min, this._max,
		 * this._valueList); console.log('should be : 0 100 [0, 20, 40, 60, 80,
		 * 100]', (this._min == 0 && this._max == 100) ? 'success' : 'failed');
		 * 
		 * this._min = -31; this._max = -3; console.log(this._min, this._max);
		 * this._reformValue(); console.log('result is :', this._min, this._max,
		 * this._valueList); console.log('should be : -35 0 [-35, -28, -21, -14,
		 * -7, 0]', (this._min == -35 && this._max == 0) ? 'success' :
		 * 'failed');
		 * 
		 * this._min = -51; this._max = 203; console.log(this._min, this._max);
		 * this._reformValue(); console.log('result is :', this._min, this._max,
		 * this._valueList); console.log('should be : -60 240 [-60, 0, 60, 120,
		 * 180, 240]', (this._min == -60 && this._max == 240) ? 'success' :
		 * 'failed');
		 * 
		 * this._min = -251; this._max = 23; console.log(this._min, this._max);
		 * this._reformValue(); console.log('result is :', this._min, this._max,
		 * this._valueList); console.log('should be : -280 70 [-280, -210, -140,
		 * -70, 0, 70]', (this._min == -280 && this._max == 70) ? 'success' :
		 * 'failed');
		 * 
		 * this.option.precision = 2; this._min = 0.23; this._max = 0.78;
		 * console.log(this._min, this._max); this._reformValue();
		 * console.log('result is :', this._min, this._max, this._valueList);
		 * console.log('should be : 0.00 1.00' + '["0.00", "0.20", "0.40",
		 * "0.60", "0.80", "1.00"]', (this._min == 0.00 && this._max == 1.00) ?
		 * 'success' : 'failed');
		 * 
		 * this._min = -12.23; this._max = -0.78; console.log(this._min,
		 * this._max); this._reformValue(); console.log('result is :',
		 * this._min, this._max, this._valueList); console.log('should be :
		 * -15.00 0.00' + '["-15.00", "-12.00", "-9.00", "-6.00", "-3.00",
		 * "0.00"]', (this._min == -15.00 && this._max == 0.00) ? 'success' :
		 * 'failed');
		 * 
		 * this._min = -0.23; this._max = 0.78; console.log(this._min,
		 * this._max); this._reformValue() console.log('result is :', this._min,
		 * this._max, this._valueList); console.log('should be : -0.30 1.20' +
		 * '["-0.30", "0.00", "0.30", "0.60", "0.90", "1.20"]', (this._min ==
		 * -0.30 && this._max == 1.20) ? 'success' : 'failed');
		 * 
		 * this._min = -1.23; this._max = 0.78; console.log(this._min,
		 * this._max); _reformValue(); console.log('result is :', this._min,
		 * this._max, this._valueList); console.log('should be : -1.50 1.00' +
		 * '["-1.50", "-1.00", "-0.50", "0.00", "0.50", "1.00"]', (this._min ==
		 * -1.50 && this._max == 1.00) ? 'success' : 'failed');
		 * 
		 * this.option.precision = 1; this._min = -2.3; this._max = 0.5;
		 * console.log(this._min, this._max); _reformValue();
		 * console.log('result is :', this._min, this._max, this._valueList);
		 * console.log('should be : -2.4 0.6' + '["-2.4", "-1.8", "-1.2",
		 * "-0.6", "0.0", "0.6"]', (this._min == -2.4 && this._max == 0.6) ?
		 * 'success' : 'failed'); --------
		 */
		_reformValue : function(scale) {
			...
			if (this._min >= 0 && this._max >= 0) {
				...
				// 正数部分的分隔数
				var partSplitNumber = Math.round(this._max / total
						* splitNumber);
				// 修正数据范围极度偏正向，留给负数一个
				partSplitNumber -= (partSplitNumber == splitNumber ? 1 : 0);
				// 修正数据范围极度偏负向，留给正数一个
				partSplitNumber += partSplitNumber === 0 ? 1 : 0;
				splitGap = (Math.ceil(Math.max(this._max / partSplitNumber,
			...
			if (precision !== 0) { // 小数
				...
				for (var i = 0; i <= splitNumber; i++) {
					this._valueList[i] = (this._valueList[i] / power)
							.toFixed(precision)
							- 0;
				}
			}
			...
			if (formatter) {
				for (var i = 0, l = this._valueList.length; i < l; i++) {
					if (typeof formatter == 'function') {
						...
						this._valueLabel.push(formatter.replace('{value}',
										this._valueList[i]));
					}
				}
			} else {
				...
				for (var i = 0, l = this._valueList.length; i < l; i++) {
					this._valueLabel
							.push(this.numAddCommas(this._valueList[i]));
				}
			}
		...
		getCoord : function(value) {
			...
			if (!this.isHorizontal()) {
				// 纵向
				result = this.grid.getYend() - (value - this._min)
						/ (this._max - this._min) * this.grid.getHeight();
			} else {
				// 横向
				result = this.grid.getX() + (value - this._min)
						/ (this._max - this._min) * this.grid.getWidth();
			}

			return result;
			// Math.floor可能引起一些偏差，但性能会更好
			/*
			 * 准确更重要 return (value == this._min || value == this._max) ? result :
			 * Math.floor(result);
			 */
		},
		...
		getCoordSize : function(value) {
			if (!this.isHorizontal()) {
				// 纵向
				return Math.abs(value / (this._max - this._min)
						* this.grid.getHeight());
			} else {
				// 横向
				return Math.abs(value / (this._max - this._min)
		...
		getValueFromCoord : function(coord) {
			...
			if (!this.isHorizontal()) {
				...
				coord = coord > this.grid.getYend()
						? this.grid.getYend()
						: coord;
				result = this._max - (coord - this.grid.getY())
						/ this.grid.getHeight() * (this._max - this._min);
			} else {
				// 横向
				coord = coord < this.grid.getX() ? this.grid.getX() : coord;
				coord = coord > this.grid.getXend()
						? this.grid.getXend()
						: coord;
				result = this._min + (coord - this.grid.getX())
						/ this.grid.getWidth() * (this._max - this._min);
			}
