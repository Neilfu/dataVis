	...
	Title.prototype = {
		...
		_buildItem : function() {
			...
			var subtextShape = {
				...
				highlightStyle : {
					color : zrColor
							.lift(this.titleOption.subtextStyle.color, 1),
					brushType : 'fill'
				},
				hoverable : false
			};
		...
		_getItemGroupLocation : function() {
			...
			var totalWidth = Math.max(zrArea.getTextWidth(text, font), zrArea
							.getTextWidth(subtext, subfont));
			var totalHeight = zrArea.getTextHeight(text, font)
					+ (subtext === '' ? 0 : (this.titleOption.itemGap + zrArea
							.getTextHeight(subtext, subfont)));
			...
			switch (this.titleOption.x) {
				case 'center' :
					...
					break;
				case 'left' :
					x = this.titleOption.padding[3]
							+ this.titleOption.borderWidth;
					break;
				case 'right' :
					x = zrWidth - totalWidth - this.titleOption.padding[1]
							- this.titleOption.borderWidth;
					break;
				default :
					x = this.titleOption.x - 0;
			...
			switch (this.titleOption.y) {
				case 'top' :
					y = this.titleOption.padding[0]
							+ this.titleOption.borderWidth;
					break;
				case 'bottom' :
					y = zrHeight - totalHeight - this.titleOption.padding[2]
							- this.titleOption.borderWidth;
					break;
				case 'center' :
		...
		refresh : function(newOption) {
			if (newOption) {
				...
				// 补全padding属性
				this.option.title.padding = this
						.reformCssArray(this.option.title.padding);
