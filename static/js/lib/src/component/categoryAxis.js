...
define(function(require) {
	...
	CategoryAxis.prototype = {
		...
		_buildAxisTick : function() {
			...
			var optGap = onGap
					? (this.getGap() / 2)
					: typeof onGap == 'undefined' ? (this.option.boundaryGap
							? (this.getGap() / 2)
							: 0) : 0;
			...
			if (this.isHorizontal()) {
				...
				for (var i = startIndex; i < dataLength; i += interval) {
					// 亚像素优化
					x = this.subPixelOptimize(this.getCoordByIndex(i)
									+ (i >= 0 ? optGap : 0), lineWidth);
					axShape = {
						_axisShape : 'axisTick',
				...
				for (var i = startIndex; i < dataLength; i += interval) {
					// 亚像素优化
					y = this.subPixelOptimize(this.getCoordByIndex(i)
									- (i >= 0 ? optGap : 0), lineWidth);
					axShape = {
						_axisShape : 'axisTick',
						zlevel : this._zlevelBase,
		...
		_buildSplitLine : function() {
			...
			if (this.isHorizontal()) {
				...
				for (var i = 0; i < dataLength; i += this._interval) {
					// 亚像素优化
					x = this.subPixelOptimize(this.getCoordByIndex(i) + optGap,
							lineWidth);
					axShape = {
						// shape : 'line',
						zlevel : this._zlevelBase,
						...
						style : {
							...
							xEnd : x,
							yEnd : ey,
							strokeColor : color[(i / this._interval)
									% colorLength],
							lineType : lineType,
							lineWidth : lineWidth
						}
				...
				for (var i = 0; i < dataLength; i += this._interval) {
					// 亚像素优化
					y = this.subPixelOptimize(this.getCoordByIndex(i) - optGap,
							lineWidth);
					axShape = {
						// shape : 'line',
						zlevel : this._zlevelBase,
		...
		getGap : function() {
			var dataLength = this.option.data.length;
			var total = this.isHorizontal() ? this.grid.getWidth() : this.grid
					.getHeight();
		...
		getCoord : function(value) {
			...
			for (var i = 0; i < dataLength; i++) {
				if (data[i] == value
						|| (typeof data[i].value != 'undefined' && data[i].value == value)) {
					if (this.isHorizontal()) {
						// 横向
					...
					/*
					 * 准确更重要 return (i === 0 || i == dataLength - 1) ? position :
					 * Math.floor(position);
					 */
				}
				position += gap;
		...
		getCoordByIndex : function(dataIndex) {
			if (dataIndex < 0) {
				...
				return position;
				/*
				 * 准确更重要 return (dataIndex === 0 || dataIndex ==
				 * this.option.data.length - 1) ? position :
				 * Math.floor(position);
				 */
			}
		},
		...
		getIndexByName : function(name) {
			...
			for (var i = 0; i < dataLength; i++) {
				if (data[i] == name
						|| (typeof data[i].value != 'undefined' && data[i].value == name)) {
					return i;
				}
			}
