...
define(function(require) {
	...
	Toolbox.prototype = {
		...
		_buildShape : function() {
						...
						case 'magicType' :
							for (var i = 0, l = feature[key].type.length; i < l; i++) {
								feature[key].title[feature[key].type[i]
										+ 'Chart'] = feature[key].title[feature[key].type[i]];
								iconName.push({
											key : key,
											name : feature[key].type[i]
													+ 'Chart'
										});
							}
							break;
			...
			if (iconName.length > 0) {
				...
				for (var i = 0, l = iconName.length; i < l; i++) {
					...
					this._featureTitle[name] = feature[key].title[name]
							|| feature[key].title;
					if (feature[key].icon) {
						this._featureIcon[name] = feature[key].icon[name]
								|| feature[key].icon;
					}
					if (feature[key].color) {
						this._featureColor[name] = feature[key].color[name]
								|| feature[key].color;
					}
				}
				...
				if (this._iconShapeMap['dataZoomReset']
		...
		_buildItem : function() {
			...
			if (toolboxOption.orient == 'horizontal') {
				...
				textPosition = this._itemGroupLocation.x / this.zr.getWidth() < 0.5
						...
						: 'left';
				/*
				 * textAlign = this._itemGroupLocation.x / this.zr.getWidth() <
				 * 0.5 ? 'right' : 'left'; textBaseline = 'top';
				 */
			}
			...
			for (var i = 0; i < iconLength; i++) {
				...
				if (this._featureIcon[this._iconList[i]]) {
					itemShape.style.image = this._featureIcon[this._iconList[i]]
				...
				switch (this._iconList[i]) {
					...
					case 'dataView' :
						if (!this._dataView) {
							...
							this._dataView = new DataView(this.ecTheme,
									this.messageCenter, this.zr, this.option,
									this.myChart);
						}
					...
					default :
						if (this._iconList[i].match('Chart')) {
							itemShape._name = this._iconList[i].replace(
									'Chart', '');
							/*
		...
		_getItemGroupLocation : function() {
			...
			switch (toolboxOption.x) {
				...
				case 'left' :
					...
					break;
				case 'right' :
					x = zrWidth - totalWidth - toolboxOption.padding[1]
			...
			switch (toolboxOption.y) {
				case 'top' :
					...
					break;
				case 'bottom' :
					y = zrHeight - totalHeight - toolboxOption.padding[2]
							- toolboxOption.borderWidth;
					break;
				case 'center' :
		...
		__onmousemove : function(param) {
			...
			if (this._zooming) {
				this._zoomShape.style.width = zrEvent.getX(param.event)
						- this._zoomShape.style.x;
				this._zoomShape.style.height = zrEvent.getY(param.event)
						- this._zoomShape.style.y;
		...
		__onmouseup : function(/* param */) {
			if (!this._zoomShape || Math.abs(this._zoomShape.style.width) < 10
			...
			if (this._zooming && this.component.dataZoom) {
				...
				var zoom = this.component.dataZoom
			...
			if (this._marking) {
				...
				this._markShape = new LineShape({
							...
							style : {
								...
								xEnd : x,
								yEnd : y,
								lineWidth : this.query(this.option,
										'toolbox.feature.mark.lineStyle.width'),
								strokeColor : this.query(this.option,
										'toolbox.feature.mark.lineStyle.color'),
								lineType : this.query(this.option,
		...
		__onMark : function(param) {
			...
			if (this._marking || this._markStart) {
				...
				setTimeout(function() {
							self.zr
									&& self.zr.on(zrConfig.EVENT.CLICK,
											self._onclick)
		...
		__onDataZoom : function(param) {
			...
			if (this._zooming || this._zoomStart) {
				...
				setTimeout(function() {
							self.zr
									&& self.zr.on(zrConfig.EVENT.MOUSEDOWN,
											self._onmousedown)
									&& self.zr.on(zrConfig.EVENT.MOUSEUP,
											self._onmouseup)
									&& self.zr.on(zrConfig.EVENT.MOUSEMOVE,
											self._onmousemove);
						}, 10);
		...
		__onRestore : function() {
			this._resetMark();
			this._resetZoom();
			this.messageCenter.dispatch(ecConfig.EVENT.RESTORE, null, null,
					this.myChart);
			return true;
		},
		...
		__onSaveAsImage : function() {
			...
			downloadDiv.style.cssText = 'position:fixed;' + 'z-index:99999;'
					+ 'display:block;' + 'top:0;left:0;'
					+ 'background-color:rgba(33,33,33,0.5);'
					+ 'text-align:center;' + 'width:100%;' + 'height:100%;'
					+ 'line-height:' + document.documentElement.clientHeight
					+ 'px;';
					...
					.setAttribute(
							...
							(saveOption.name
									...
									: (this.option.title && (this.option.title.text || this.option.title.subtext))
											...
											: 'ECharts')
									+ '.' + imgType);
			downloadLink.innerHTML = '<img style="vertical-align:middle" src="'
					+ image
					+ '" title="'
					+ (!!(window.attachEvent && navigator.userAgent
							.indexOf('Opera') === -1)
							? '右键->图片另存为'
			...
			/*
			 * function _saveImageForIE() { window.win = window.open(image);
			 * win.document.execCommand("SaveAs"); win.close() }
			 */
			return;
		},
		...
		reset : function(newOption, isRestore) {
				...
				while (len--) {
					...
					if (this._magicMap[oriType]) {
						axis = newOption.xAxis instanceof Array
								? newOption.xAxis[newOption.series[len].xAxisIndex
										|| 0]
								: newOption.xAxis;
						...
						axis = newOption.yAxis instanceof Array
								? newOption.yAxis[newOption.series[len].yAxisIndex
										|| 0]
								: newOption.yAxis;
						...
						// 避免不同类型图表类型的样式污染
						newOption.series[len].__itemStyle = zrUtil
								.clone(	newOption.series[len].itemStyle || {});
					}

					if (this._magicMap[_MAGICTYPE_STACK]
							|| this._magicMap[_MAGICTYPE_TILED]) {
			...
			if (zoomOption && zoomOption.show) {
				var start = typeof zoomOption.start != 'undefined'
						&& zoomOption.start >= 0 && zoomOption.start <= 100
						? zoomOption.start
						: 0;
				var end = typeof zoomOption.end != 'undefined'
						&& zoomOption.end >= 0 && zoomOption.end <= 100
						? zoomOption.end
						: 100;
				if (start > end) {
					// 大小颠倒自动翻转
		...
		getMagicOption : function() {
			...
			if (this._magicType[ecConfig.CHART_TYPE_LINE]
				...
				// 图表类型有切换
				var boundaryGap = this._magicType[ecConfig.CHART_TYPE_LINE]
						? false
						: true;
				for (var i = 0, l = this.option.series.length; i < l; i++) {
					if (this._magicMap[this.option.series[i].type]) {
						...
						this.option.series[i].itemStyle = zrUtil
								.clone(	this.option.series[i].__itemStyle);

						axis = this.option.xAxis instanceof Array
								? this.option.xAxis[this.option.series[i].xAxisIndex
										|| 0]
								: this.option.xAxis;
						if (axis && (axis.type || 'category') == 'category') {
						...
						axis = this.option.yAxis instanceof Array
								? this.option.yAxis[this.option.series[i].yAxisIndex
										|| 0]
								: this.option.yAxis;
						if (axis && axis.type == 'category') {
							axis.boundaryGap = boundaryGap
			...
			if (this._magicType[_MAGICTYPE_STACK]
					|| this._magicType[_MAGICTYPE_TILED]) {
				// 有堆积平铺切换
				for (var i = 0, l = this.option.series.length; i < l; i++) {
					if (this._magicType[_MAGICTYPE_STACK]) {
						// 启用堆积
		...
		refresh : function(newOption) {
			if (newOption) {
				...
				// 补全padding属性
				newOption.toolbox.padding = this
						.reformCssArray(newOption.toolbox.padding);
				this.option = newOption;
