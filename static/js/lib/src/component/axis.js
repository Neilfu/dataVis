 ...
 * 
 * 直角坐标系中坐标轴数组，数组中每一项代表一条横轴（纵轴）坐标轴。 标准（1.0）中规定最多同时存在2条横轴和2条纵轴
...
define(function(require) {
	 ...
	 *            option 图表选项
	 * @param {string=}
	 *            option.xAxis.type 坐标轴类型，横轴默认为类目型'category'
	 * @param {string=}
	 *            option.yAxis.type 坐标轴类型，纵轴默认为类目型'value'
	 * @param {Object}
	 *            component 组件
	 * @param {string}
	 *            axisType 横走or纵轴
	...
	Axis.prototype = {
		...
		axisBase : {
			...
			_buildAxisLine : function() {
				...
				if (this.option.name !== '') {
					...
					axShape.style.textFont = this
							.getFont(this.option.nameTextStyle);
				...
				if (this.isHorizontal()) {
					// 横向布局，优化y
					axShape.style.yStart = axShape.style.yEnd = this
							.subPixelOptimize(axShape.style.yEnd, lineWidth);
				} else {
					// 纵向布局，优化x
					axShape.style.xStart = axShape.style.xEnd = this
							.subPixelOptimize(axShape.style.xEnd, lineWidth);
				}
			...
			_axisLabelClickable : function(clickable, axShape) {
				if (clickable) {
					ecData.pack(axShape, undefined, -1, undefined, -1,
							axShape.style.text);
			...
			refixAxisShape : function(zeroX, zeroY) {
				...
				if (this.isHorizontal() && typeof zeroY != 'undefined') {
					...
					for (var i = 0, l = this.shapeList.length; i < l; i++) {
						if (this.shapeList[i]._axisShape == 'axisLine') {
							this.shapeList[i].style.yStart = this.shapeList[i].style.yEnd = this
									.subPixelOptimize(zeroY,
				...
				if (!this.isHorizontal() && typeof zeroX != 'undefined') {
					...
					for (var i = 0, l = this.shapeList.length; i < l; i++) {
						if (this.shapeList[i]._axisShape == 'axisLine') {
							this.shapeList[i].style.xStart = this.shapeList[i].style.xEnd = this
									.subPixelOptimize(zeroX,
											this.shapeList[i].stylelineWidth);
							this.zr.modShape(this.shapeList[i].id);
						} else if (this.shapeList[i]._axisShape == 'axisTick') {
			...
			isHorizontal : function() {
				return this.option.position == 'bottom'
		...
		refresh : function(newOption) {
			...
			var len = Math.max((axisOption && axisOption.length || 0),
			...
			for (var i = 0; i < len; i++) {
				...
				if (this._axisList[i]) {
					this._axisList[i].refresh
							&& this._axisList[i].refresh(axisOption
											? axisOption[i]
											: false, this.series);
				} else if (axisOption && axisOption[i]) {
					this._axisList[i] = axisOption[i].type == 'category'
							? new CategoryAxis(this.ecTheme,
									this.messageCenter, this.zr, axisOption[i],
									this.myChart, this.axisBase)
							: new ValueAxis(this.ecTheme, this.messageCenter,
									this.zr, axisOption[i], this.myChart,
									this.axisBase, this.series);
