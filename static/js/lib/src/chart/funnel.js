...
define(function(require) {
	...
	Funnel.prototype = {
		...
		_buildSingleFunnel : function(seriesIndex) {
			...
			var gap = serie.gap;
			var height = total > 1 ? (location.height - (total - 1) * gap)
					/ total : location.height;
			var width;
			var lastY = location.y;
			var lastWidth = serie.sort == 'descending' ? this._getItemWidth(
					seriesIndex, selectedData[0].value) : number.parsePercent(
					serie.minSize, location.width);
			...
			for (var i = 0, l = selectedData.length; i < l; i++) {
				...
				if (this.selectedMap[itemName] && !isNaN(selectedData[i].value)) {
					width = i <= l - 2
							? this._getItemWidth(seriesIndex, selectedData[i
											+ next].value)
							: serie.sort == 'descending'
									? number.parsePercent(serie.minSize,
											location.width)
									: number.parsePercent(serie.maxSize,
											location.width);
					this._buildItem(seriesIndex, selectedData[i]._index, legend
									? legend.getColor(itemName)
									: this.zr.getColor(selectedData[i]._index),
							centerX - lastWidth / 2, lastY, lastWidth, width,
							height);
					lastY += height + gap;
					lastWidth = width;
		...
		_buildFunnelCase : function(seriesIndex) {
			...
			if (this.deepQuery([serie, this.option], 'calculable')) {
				...
				var centerX = location.centerX;
				var minWidth = number.parsePercent(serie.minSize,
						location.width)
						/ 2;
				var maxWidth = number.parsePercent(serie.maxSize,
						location.width)
						/ 2;
				var gap = 10;
				var funnelCase = {
		...
		_getLocation : function(seriesIndex) {
			...
			if (typeof gridOption.height == 'undefined') {
				height = zrHeight - y
						- this.parsePercent(gridOption.y2, zrHeight);
			} else {
		...
		_mapData : function(seriesIndex) {
			...
			if (serie.sort != 'none') {
				funnelData.sort(serie.sort == 'descending'
						? numDescending
						: numAscending)
			}
		 ...
		 * 构建单个扇形及指标
		 */
		_buildItem : function(seriesIndex, dataIndex, defaultColor, x, y,
				topWidth, bottomWidth, height) {
			var series = this.series;
			var serie = series[seriesIndex];
			var data = serie.data[dataIndex];
			// 漏斗
			var polygon = this.getPolygon(seriesIndex, dataIndex, defaultColor,
					x, y, topWidth, bottomWidth, height);
			ecData.pack(polygon, series[seriesIndex], seriesIndex,
					series[seriesIndex].data[dataIndex], dataIndex,
					series[seriesIndex].data[dataIndex].name);
			this.shapeList.push(polygon);

			// 文本标签
			var label = this.getLabel(seriesIndex, dataIndex, defaultColor, x,
					y, topWidth, bottomWidth, height);
			...
			var labelLine = this.getLabelLine(seriesIndex, dataIndex,
					defaultColor, x, y, topWidth, bottomWidth, height);
		 ...
		 * 构建扇形
		 */
		getPolygon : function(seriesIndex, dataIndex, defaultColor, x, y,
				topWidth, bottomWidth, height) {
			var serie = this.series[seriesIndex];
			var data = serie.data[dataIndex];
			...
			var polygon = {
				...
				style : {
					pointList : [
							[x, y],
							[x + topWidth, y],
							[x + topWidth - (topWidth - bottomWidth) / 2,
									y + height],
		 ...
		 */
		getLabel : function(seriesIndex, dataIndex, defaultColor, x, y,
				topWidth, bottomWidth, height) {
			var serie = this.series[seriesIndex];
			var data = serie.data[dataIndex];
			var location = this._paramsMap[seriesIndex].location;
			...
			if (labelControl.position == 'inner'
				...
				textAlign = 'center';
				textX = x + topWidth / 2;
				if (Math.max(topWidth, bottomWidth) / 2 > zrArea.getTextWidth(
						text, textFont)) {
					textColor = '#fff';
				} else {
				...
				textX = lineLength == 'auto'
						? (location.x - 10)
						: (location.centerX - Math.max(topWidth, bottomWidth)
								/ 2 - lineLength);
			} else {
		 ...
		 * 需要显示则会有返回构建好的shape，否则返回undefined
		 */
		getLabelLine : function(seriesIndex, dataIndex, defaultColor, x, y,
				topWidth, bottomWidth, height) {
			var serie = this.series[seriesIndex];
			var data = serie.data[dataIndex];
			var location = this._paramsMap[seriesIndex].location;
