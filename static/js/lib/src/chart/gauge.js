...
define(function(require) {
	...
	Gauge.prototype = {
		...
		_buildAxisLine : function(seriesIndex) {
			...
			var lineWidth = this
					.parsePercent(lineStyle.width, params.radius[1]);
			...
			for (var i = 0, l = colorArray.length; i < l; i++) {
				newAngle = startAngle - totalAngle * (colorArray[i][0] - min)
						/ total;
			...
			for (var i = 0; i <= splitNumber; i++) {
				...
				this.shapeList.push(new LineShape({
							...
							style : {
								...
								strokeColor : color == 'auto' ? this._getColor(
										seriesIndex, min + total / splitNumber
												* i) : color,
				...
				this.shapeList.push(new LineShape({
							...
							style : {
								...
								strokeColor : color == 'auto'
										? this._getColor(seriesIndex, min
														+ total / l * i)
										: color,
								lineType : lineStyle.type,
								lineWidth : lineStyle.width,
		...
		_buildAxisLabel : function(seriesIndex) {
			...
			var r0 = params.radius[1]
					- this.parsePercent(serie.splitLine.length,
							params.radius[1]) - 10;
			var axShape;
			var angle;
			var sinAngle;
			...
			for (var i = 0; i <= splitNumber; i++) {
				...
				this.shapeList.push(new TextShape({
							...
							style : {
								...
								color : color == 'auto' ? this._getColor(
										seriesIndex, value) : color,
								text : this._getLabelText(
										serie.axisLabel.formatter, value),
		...
		_buildPointer : function(seriesIndex) {
			...
			var angle = (params.startAngle - params.totalAngle / total
					* (value - serie.min))
					* Math.PI / 180;
			var color = pointer.color == 'auto' ? this._getColor(seriesIndex,
					value) : pointer.color;
			...
			ecData.pack(pointShape, this.series[seriesIndex], seriesIndex,
					this.series[seriesIndex].data[0], 0,
					this.series[seriesIndex].data[0].name, value);
			...
			if (name !== '') {
				...
				this.shapeList.push(new TextShape({
							zlevel : this._zlevelBase
									+ (Math.abs(x - params.center[0]) + Math
											.abs(y - params.center[1])) < textStyle.fontSize
									* 2 ? 2 : 1,
							hoverable : false,
							style : {
								x : x,
								y : y,
								color : textColor == 'auto' ? this
										._getColor(seriesIndex) : textColor,
								text : name,
								textAlign : 'center',
		...
		_buildDetail : function(seriesIndex) {
			...
			this.shapeList.push(new RectangleShape({
				zlevel : this._zlevelBase
						+ (Math.abs(x + detail.width / 2 - params.center[0]) + Math
								.abs(y + detail.height / 2 - params.center[1])) < textStyle.fontSize
						? 2
						: 1,
				hoverable : false,
				style : {
					...
					brushType : 'both',
					color : color == 'auto' ? this
							._getColor(seriesIndex, value) : color,
					...
					textFont : this.getFont(textStyle),
					textPosition : 'inside',
					textColor : textColor == 'auto' ? this._getColor(
							seriesIndex, value) : textColor
				}
			}));
		},
