	...
	Chord.prototype = {
		...
		_buildShape : function() {
			...
			this.center = [
					...
					this
							.parsePercent(this.chordSerieSample.center[1],
									zrHeight)];
			var fixSize = this.chordSerieSample.itemStyle.normal.chordStyle.lineStyle.width
					- this.chordSerieSample.itemStyle.normal.lineStyle.width;
			this.strokeFix = (fixSize / _devicePixelRatio) / this.innerRadius
					/ Math.PI * 180;
			...
			var subGroupAngles = this.dataMat.div(this.dataMat.sum(1).reshape(
							groupNumber, 1));
			subGroupAngles = subGroupAngles.mul(groupAngles.sub(this.strokeFix
							* 2).reshape(groupNumber, 1));
			...
			switch (this.sortSubGroups) {
				case 'ascending' :
				case 'descending' :
					var subGroupIndices = subGroupAngles.argsort(1,
							this.sortSubGroups);
					subGroupAngles['sort'](1, this.sortSubGroups);
					break;
				default :
					var subGroupIndices = NDArray.range(subGroupNumber)
							.reshape(1, subGroupNumber).repeat(groupNumber, 0);
			}
			...
			var sectorAngles = [];
			var chordAngles = new NDArray(groupNumber, subGroupNumber)
					.toArray();
			var values = [];
			var start = 0;
			var end = 0;
			...
			// reset data
			this.chordShapes = new NDArray(groupNumber, groupNumber,
					serieNumber).toArray();
			this.sectorShapes = [];

			this._buildSectors(sectorAngles, values);

			chordAngles = new NDArray(chordAngles).reshape(groupNumber,
					groupNumber, serieNumber, 2).toArray();
			this._buildChords(chordAngles, this.dataMat.reshape(shape)
							.toArray());

			var res = this.normalizeValue(values);
			if (this.showScale) {
				this._buildScales(res[0], res[1], sectorAngles,
						new NDArray(res[0]).sum()
								/ (360 - this.padding * groupNumber));
			}
		...
		_buildSectors : function(angles, data) {
			...
			var showLabel = this.query(this.chordSerieSample,
					'itemStyle.normal.label.show');
			var labelColor = this.query(this.chordSerieSample,
					'itemStyle.normal.label.color');
			var rotateLabel = this.query(this.chordSerieSample,
					'itemStyle.normal.label.rotate');
			var labelDistance = this.query(this.chordSerieSample,
					'itemStyle.normal.label.distance');
			...
			function createMouseOver(idx) {
				return function() {
					...
					timeout = setTimeout(function() {
						for (var i = 0; i < len; i++) {
							self.sectorShapes[i].style.opacity = i === idx
									? 1
									: 0.1;
							...
							for (var j = 0; j < len; j++) {
								for (var k = 0; k < len2; k++) {
									...
									if (chordShape) {
										chordShape.style.opacity = (i === idx || j === idx)
												? 0.5
												: 0.03;
			...
			for (var i = 0; i < len; i++) {
				...
				var angle = angles[i];
				var _start = (this.clockWise ? (360 - angle[1]) : angle[0])
						+ this.startAngle;
				var _end = (this.clockWise ? (360 - angle[0]) : angle[1])
						+ this.startAngle;
				...
				sector.style.lineWidth = this.deepQuery([group,
								this.chordSerieSample],
						'itemStyle.normal.lineStyle.width');
				sector.highlightStyle.lineWidth = this.deepQuery([group,
								this.chordSerieSample],
						'itemStyle.emphasis.lineStyle.width');
				sector.style.strokeColor = this.deepQuery([group,
								this.chordSerieSample],
						'itemStyle.normal.lineStyle.color');
				sector.highlightStyle.strokeColor = this.deepQuery([group,
								this.chordSerieSample],
				...
				ecData.pack(sector, this.chordSeries[0], 0, data[i], i,
						group.name);
				if (showLabel) {
					...
					var distance = this.showScaleText
							? 35 + labelDistance
							: labelDistance;
					...
					if (rotateLabel) {
						labelShape.rotation = isRightSide ? halfAngle : Math.PI
								+ halfAngle;
						if (isRightSide) {
					...
					labelShape.style.textColor = this.deepQuery([group,
									this.chordSerieSample],
			...
			for (var i = 0; i < len; i++) {
				for (var j = 0; j < len; j++) {
					for (var k = 0; k < len2; k++) {
						...
						ecData.pack(chord, this.chordSeries[k], k,
								dataArr[i][j][k], i + '-' + j,
								this.groups[i].name, this.groups[j].name,
								dataArr[j][i][k]);
		...
		_buildScales : function(values, unitPostfix, angles, unitValue) {
			for (var i = 0; i < angles.length; i++) {
				...
				while (scaleAngle < subEndAngle) {
					var thelta = ((this.clockWise
							? (360 - scaleAngle)
							: scaleAngle) + this.startAngle)
							/ 180 * Math.PI;
					var v = [Math.cos(thelta), -Math.sin(thelta)];
					var start = vec2.scale([], v, this.outerRadius + 1);
					vec2.add(start, start, this.center);
					var end = vec2.scale([], v, this.outerRadius
									+ this.scaleLineLength);
				...
				while (scaleTextAngle < subEndAngle) {
					...
					var textShape = {
						...
						style : {
							x : isRightSide
									? this.outerRadius + this.scaleLineLength
											+ 4
									: -this.outerRadius - this.scaleLineLength
											- 4,
							y : 0,
		...
		refresh : function(newOption) {
			...
			/*
			 * this.legend; this.getColor; this.isSelected;
			 * this.chordSerieSample;
			 */
			// Config
			this.chordSeries = [];
			/*
			 * this.groups; this.startAngle; this.clockWise; this.innerRadius;
			 * this.outerRadius; this.padding; this.sortGroups;
			 * this.sortSubGroups; this.center; this.showScale;
			 * this.showScaleText; this.dataMat;
			 */
			this.strokeFix = 0;
			// Adjacency matrix
			this.sectorShapes = [];
				...
				this.getColor = function(key) {
					...
					for (var i = 0; i < this.chordSeries.length; i++) {
						if (this.chordSeries[i].name === key) {
							colorMap[key] = this.query(this.chordSeries[i],
					...
					if (!colorMap[key]) {
						...
						for (var i = 0; i < len; i++) {
							if (this.groups[i].name === key) {
								colorMap[key] = this.query(this.groups[i],
										'itemStyle.normal.color');
								break;
							}
						}
