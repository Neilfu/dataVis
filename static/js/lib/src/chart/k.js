...
define(function(require) {
	...
	K.prototype = {
		...
		_buildKLine : function(seriesArray, pointList) {
			...
			for (var sIdx = 0, len = seriesArray.length; sIdx < len; sIdx++) {
				...
				if (serie.type == ecConfig.CHART_TYPE_K
					...
					// 多级控制
					queryTarget = serie;
					nLineWidth = this.query(queryTarget,
							'itemStyle.normal.lineStyle.width');
					nLineColor = this.query(queryTarget,
							'itemStyle.normal.lineStyle.color');
					nLineColor0 = this.query(queryTarget,
							'itemStyle.normal.lineStyle.color0');
					nColor = this.query(queryTarget, 'itemStyle.normal.color');
					nColor0 = this
							.query(queryTarget, 'itemStyle.normal.color0');

					eLineWidth = this.query(queryTarget,
							'itemStyle.emphasis.lineStyle.width');
					eLineColor = this.query(queryTarget,
							'itemStyle.emphasis.lineStyle.color');
					eLineColor0 = this.query(queryTarget,
							'itemStyle.emphasis.lineStyle.color0');
					eColor = this
							.query(queryTarget, 'itemStyle.emphasis.color');
					eColor0 = this.query(queryTarget,
							'itemStyle.emphasis.color0');

					/*
					 * pointlist=[ 0 x, 1 width, 2 y0, 3 y1, 4 y2, 5 y3, 6
					 * dataIndex, 7 categoryName ]
					 */
					for (var i = 0, l = seriesPL.length; i < l; i++) {
						...
						this.shapeList
								.push(this
										._getCandle(seriesIndex, // seriesIndex
												...
												candleType
														? (this
																.query(
																		// 阳
																		queryTarget,
																		'itemStyle.normal.color') || nColor)
														: (this
																.query(
																		// 阴
																		queryTarget,
																		'itemStyle.normal.color0') || nColor0),
												...
												candleType
														? (this
																.query(
																		// 阳
																		queryTarget,
																		'itemStyle.normal.lineStyle.color') || nLineColor)
														: (this
																.query(
																		// 阴
																		queryTarget,
																		'itemStyle.normal.lineStyle.color0') || nLineColor0),
		 ...
		 * 生成K线图上的图形
		 */
		_getCandle : function(seriesIndex, dataIndex, name, x, width, y0, y1,
				y2, y3, nColor, nLinewidth, nLineColor, eColor, eLinewidth,
				eLineColor) {
			var series = this.series;
			var itemShape = {
			...
			ecData.pack(itemShape, series[seriesIndex], seriesIndex,
			...
			for (var i = 0, l = this.shapeList.length; i < l; i++) {
				...
				if (aniMap[seriesIndex] && !aniMap[seriesIndex][3]) {
					...
					if (this.shapeList[i].type == 'candle') {
						...
						dx = this.component.xAxis
								.getAxis(serie.xAxisIndex || 0).getGap();
