	...
	Map.prototype = {
		...
		_buildShape : function() {
			...
			for (var i = 0, l = series.length; i < l; i++) {
				if (series[i].type == ecConfig.CHART_TYPE_MAP) { // map
					...
					this._scaleLimitMap[mapType] = this._scaleLimitMap[mapType]
							|| {};
					series[i].scaleLimit
							&& zrUtil.merge(this._scaleLimitMap[mapType],
									series[i].scaleLimit, true);

					this._roamMap[mapType] = series[i].roam
							|| this._roamMap[mapType];
					this._needRoam = this._needRoam || this._roamMap[mapType];

					this._nameMap[mapType] = this._nameMap[mapType] || {};
					series[i].nameMap
							&& zrUtil.merge(this._nameMap[mapType],
									series[i].nameMap, true);
					...
					this.selectedMap[seriesName] = legend ? legend
							.isSelected(seriesName) : true;
						...
						for (var j = 0, k = data.length; j < k; j++) {
							...
							for (var key in data[j]) {
								if (key != 'value') {
									...
									typeof valueData[mapType][name].value == 'undefined'
			...
			for (var mt in valueData) {
				...
				if (this._mapDataMap[mt].mapData) {
					...
					// 特殊区域
					this._specialArea[mt] = _mapParams[mt.replace(/\|.*/, '')].specialArea
							|| this._specialArea[mt];
					_mapParams[mt.replace(/\|.*/, '')].getGeoJson(this
									._mapDataCallback(mt, valueData[mt],
											mapSeries[mt]));
				}
			}
		},
		...
		_getProjectionData : function(mapType, mapData, mapSeries) {
			...
			if (!this._mapDataMap[mapType].hasRoam) {
				// 第一次或者发生了resize，需要判断
				transform = this._getTransform(bbox, mapSeries,
			...
			for (var i = 0, l = pathArray.length; i < l; i++) {
				/*
				 * for test console.log( mapData.features[i].properties.cp, //
				 * 经纬度度 pathArray[i].cp // 平面坐标 ); console.log(
				 * this.pos2geo(mapType, pathArray[i].cp), // 平面坐标转经纬度
				 * this.geo2pos(mapType, mapData.features[i].properties.cp) )
				 */
				province.push(this._getSingleProvince(mapType, pathArray[i],
						position));
			}

			if (this._specialArea[mapType]) {
				for (var area in this._specialArea[mapType]) {
					province.push(this._getSpecialProjectionData(mapType,
							mapData, area, this._specialArea[mapType][area],
							position));
				}
			...
			// 中国地图加入南海诸岛
			if (mapType == 'china') {
				var leftTop = this.geo2pos(mapType, _geoCoord['南海诸岛']
								|| _mapParams['南海诸岛'].textCoord);
				// scale.x : width = 10.51 : 64
				var scale = transform.scale.x / 10.5;
				var textPosition = [32 * scale + leftTop[0],
						83 * scale + leftTop[1]];
				if (_textFixed['南海诸岛']) {
		 ...
		 * 特殊地区投射数据
		 */
		_getSpecialProjectionData : function(mapType, mapData, areaName,
				mapSize, position) {
		...
		_getSingleProvince : function(mapType, path, position) {
			...
			if (_geoCoord[name]) {
				...
				textPosition = [path.cp[0] + textFixed[0],
		...
		_buildMap : function(mapType, mapData, valueData, mapSeries) {
			...
			for (var i = 0, l = mapData.length; i < l; i++) {
				...
				if (data) {
					...
					for (var j = 0, k = data.seriesIndex.length; j < k; j++) {
						...
						if (legend
								...
								&& legend
										.hasColor(series[data.seriesIndex[j]].name)) {
							this.shapeList.push(new CircleShape({
								zlevel : this._zlevelBase + 1,
								position : zrUtil.clone(style.position),
								_mapType : mapType,
								/*
								 * _geo : this.pos2geo( mapType, [style.textX +
								 * 3 + j * 7, style.textY - 10] ),
								 */
								style : {
									x : style.textX + 3 + j * 7,
									y : style.textY - 10,
									r : 3,
									color : legend
											.getColor(series[data.seriesIndex[j]].name)
								},
								hoverable : false
							}));
						}
				...
				// 值域控件控制
				color = (dataRange && !isNaN(value)) ? dataRange
						.getColor(value) : null;

				// 常规设置
				style.color = style.color
						|| color
						|| this.getItemStyleColor(this.deepQuery(queryTarget,
										'itemStyle.normal.color'),
								data.seriesIndex, -1, data)
						|| this.deepQuery(queryTarget,
				...
				// 高亮
				highlightStyle.color = this.getItemStyleColor(this.deepQuery(
								queryTarget, 'itemStyle.emphasis.color'),
						data.seriesIndex, -1, data)
						|| this.deepQuery(queryTarget,
								'itemStyle.emphasis.areaStyle.color')
						|| style.color;
				highlightStyle.strokeColor = this.deepQuery(queryTarget,
						'itemStyle.emphasis.borderColor')
						|| style.strokeColor;
				highlightStyle.lineWidth = this.deepQuery(queryTarget,
						'itemStyle.emphasis.borderWidth')
						|| style.lineWidth;

				style.brushType = highlightStyle.brushType = style.brushType
						|| 'both';
				style.lineJoin = highlightStyle.lineJoin = 'round';
				style._name = highlightStyle._name = name;

				font = this.deepQuery(queryTarget,
				...
				textShape = {
					...
					style : {
						...
						x : style.textX,
						y : style.textY,
						text : this.getLabelText(name, value, queryTarget,
								'normal'),
						_name : name,
						textAlign : 'center',
						color : this.deepQuery(queryTarget,
								'itemStyle.normal.label.show')
								? this
										.deepQuery(queryTarget,
				...
				if (this
						.deepQuery(queryTarget, 'itemStyle.emphasis.label.show')) {
					textShape.highlightStyle.text = this.getLabelText(name,
							value, queryTarget, 'emphasis');
				...
				switch (shape.style.shapeType) {
					...
					default :
						...
						shape.pathArray = shape
								._parsePathData(shape.style.path);
						break;
				}

				if (this._selectedMode[mapType] && this._selected[name]
		...
		_buildMark : function(mapType, mapSeries) {
			...
			var position = [this._mapDataMap[mapType].transform.left,
		...
		getMarkCoord : function(seriesIndex, mpData) {
			return (mpData.geoCoord || _geoCoord[mpData.name]) ? this.geo2pos(
					this._seriesIndexToMapType[seriesIndex], mpData.geoCoord
							|| _geoCoord[mpData.name]) : [0, 0];
		},
		...
		getLabelText : function(name, value, queryTarget, status) {
			var formatter = this.deepQuery(queryTarget, 'itemStyle.' + status
							+ '.label.formatter');
			if (formatter) {
				if (typeof formatter == 'function') {
					...
					formatter = formatter.replace('{a}', '{a0}').replace('{b}',
							'{b0}');
					formatter = formatter.replace('{a0}', name).replace('{b0}',
							value);
					return formatter;
				}
			} else {
		...
		_findMapTypeByPos : function(mx, my) {
			...
			for (var mapType in this._mapDataMap) {
				...
				if (mx >= left && mx <= (left + width) && my >= top
						&& my <= (top + height)) {
					return mapType;
				}
			}
		...
		__onmousewheel : function(param) {
			...
			if (mapType) {
				...
				for (var i = 0, l = this.shapeList.length; i < l; i++) {
					if (this.shapeList[i]._mapType == mapType) {
						...
						if (this.shapeList[i].type == 'path'
							...
							geoAndPos = this.geo2pos(mapType,
									this.shapeList[i]._geo[0]);
							this.shapeList[i].style.xStart = geoAndPos[0];
							this.shapeList[i].style.yStart = geoAndPos[1];
							geoAndPos = this.geo2pos(mapType,
									this.shapeList[i]._geo[1]);
							this.shapeList[i].style.xEnd = geoAndPos[0];
							this.shapeList[i].style.yEnd = geoAndPos[1];
						} else if (this.shapeList[i].type == 'icon') {
							geoAndPos = this.geo2pos(mapType,
									this.shapeList[i]._geo);
							this.shapeList[i].style.x = this.shapeList[i].style._x = geoAndPos[0]
									- this.shapeList[i].style.width / 2;
							this.shapeList[i].style.y = this.shapeList[i].style._y = geoAndPos[1]
									- this.shapeList[i].style.height / 2;
						} else {
							geoAndPos = this.geo2pos(mapType,
									this.shapeList[i]._geo);
				...
				this.messageCenter.dispatch(ecConfig.EVENT.MAP_ROAM,
						param.event, {
							type : 'scale'
						}, this.myChart);
			}
		...
		__onmousedown : function(param) {
			...
			if (mapType) {
				...
				var self = this;
				setTimeout(function() {
							self.zr.on(zrConfig.EVENT.MOUSEMOVE,
									self._onmousemove);
						}, 100);
			}
		...
		__onmousemove : function(param) {
			...
			this.messageCenter.dispatch(ecConfig.EVENT.MAP_ROAM, param.event, {
						type : 'move'
					}, this.myChart);
		...
		onclick : function(param) {
			...
			this.messageCenter.dispatch(ecConfig.EVENT.MAP_SELECTED,
					param.event, {
			...
			var self = this;
			setTimeout(function() {
						self.zr.trigger(zrConfig.EVENT.MOUSEMOVE, param.event);
					}, 100)
		},
		...
		getMapPosition : function(mapType) {
			...
			return [this._mapDataMap[mapType].transform.left,
					this._mapDataMap[mapType].transform.top];
		},

		/*
		 * appendShape : function (mapType, shapeList) { shapeList = shapeList
		 * instanceof Array ? shapeList : [shapeList]; for (var i = 0, l =
		 * shapeList.length; i < l; i++) { if (typeof shapeList[i].zlevel ==
		 * 'undefined') { shapeList[i].zlevel = this._zlevelBase + 1; }
		 * shapeList[i]._mapType = mapType; this.shapeList.push(shapeList[i]);
		 * this.zr.addShape(shapeList[i]); } this.zr.refresh(); },
		 */
