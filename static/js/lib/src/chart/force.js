...
define(function(require) {
	...
	function Force(ecTheme, messageCenter, zr, option, myChart) {
		...
		function _buildShape() {
			...
			for (var i = 0, l = series.length; i < l; i++) {
				...
				if (serie.type === ecConfig.CHART_TYPE_FORCE) {
					...
					// 系列图例开关
					self.selectedMap[serieName] = legend ? legend
					...
					for (var j = 0, len = categories.length; j < len; j++) {
						if (categories[j].name) {
							if (legend) {
								self.selectedMap[j] = legend
										.isSelected(categories[j].name);
							} else {
					...
					linkStyle = self.query(serie, 'itemStyle.normal.linkStyle');
					linkEmphasisStyle = self.query(serie,
							'itemStyle.emphasis.linkStyle');
					nodeStyle = self.query(serie, 'itemStyle.normal.nodeStyle');
					nodeEmphasisStyle = self.query(serie,
		...
		function _buildNodeShapes(nodes, minRadius, maxRadius) {
			...
			radius = narr.map(minRadius, maxRadius).toArray();
			var max = narr.max();
			if (max !== 0) {
			...
			for (var i = 0; i < l; i++) {
				...
				if (nodeInitialPos[node.name] !== undefined) {
					...
					initPos = _randomInSquare(viewportWidth / 2, viewportHeight
									/ 2, initSize);
				}
				var x = initPos[0];
				var y = initPos[1];
				...
				if (self.query(forceSerie, 'itemStyle.normal.label.show')) {
					...
					labelStyle = self.query(forceSerie,
				...
				if (typeof(node.category) !== 'undefined') {
					...
					if (category) {
						...
						if (style) {
							...
							if (style.emphasis) {
								zrUtil.merge(shape.highlightStyle,
										style.emphasis, true);
							}
						}
					}
				...
				if (typeof(node.itemStyle) !== 'undefined') {
					...
					if (style.normal) {
						zrUtil
								.merge(shape.highlightStyle, style.emphasis,
										true);
					}
				}
			...
			for (var i = 0; i < l; i++) {
				...
				if (typeof(link.itemStyle) !== 'undefined') {
					...
					if (link.itemStyle.emphasis) {
						zrUtil.merge(linkShape.highlightStyle,
								link.itemStyle.emphasis, true);
					}
				}
		...
		function _updateLinkShapes() {
			...
			for (var i = 0, len = filteredLinks.length; i < len; i++) {
				...
				if (forceSerie.linkSymbol) {
					...
					vec2.scaleAndAdd(arrowShape.position, arrowShape.position,
							v, targetShape.style.r + 2);
					var angle;
					if (v[1] < 0) {
		...
		function _update(stepTime) {
			...
			for (var i = 0; i < len; i++) {
				for (var j = i + 1; j < len; j++) {
					...
					// 节点1受到的力
					vec2.scaleAndAdd(nodeForces[i], nodeForces[i], v12,
							-forceFactor);
					// 节点2受到的力
					vec2.scaleAndAdd(nodeForces[j], nodeForces[j], v12,
							forceFactor);
				}
			}
			// 计算节点之间引力
			for (var i = 0, l = filteredLinks.length; i < l; i++) {
				...
				// 节点1受到的力
				vec2
						.scaleAndAdd(nodeForces[s], nodeForces[s], v12,
								forceFactor);
				// 节点2受到的力
				vec2.scaleAndAdd(nodeForces[t], nodeForces[t], v12,
						-forceFactor);
			}
			// 到质心的向心力
			for (var i = 0, l = filteredNodes.length; i < l; i++) {
				...
				vec2
						.scaleAndAdd(nodeForces[i], nodeForces[i], v12,
								forceFactor);
			}
			var velocity = [];
			// 计算位置(verlet积分)
			for (var i = 0, l = nodePositions.length; i < l; i++) {
				...
				__P[0] = p[0];
				__P[1] = p[1];
				vec2.scaleAndAdd(velocity, velocity, nodeForces[i], stepTime
								/ nodeMasses[i]);
				// Damping
	...
	/*
	 * function _randomInCircle(x, y, radius) { var theta = Math.random() *
	 * Math.PI * 2; var r = radius * Math.random(); return [ Math.cos(theta) * r +
	 * x, Math.sin(theta) * r + y ]; }
	 */

	function _randomInSquare(x, y, size) {
		return [(Math.random() - 0.5) * size + x,
