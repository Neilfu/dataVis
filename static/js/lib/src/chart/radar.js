...
define(function(require) {
			...
			function Radar(ecTheme, messageCenter, zr, option, myChart) {
				// 基类
				ComponentBase.call(this, ecTheme, messageCenter, zr, option,
						myChart);
				// 图表基类
				ChartBase.call(this);
			...
			Radar.prototype = {
					...
					for (var i = 0, l = series.length; i < l; i++) {
						if (series[i].type == ecConfig.CHART_TYPE_RADAR) {
							...
							if (this.selectedMap[serieName]) {
								...
								if (this.deepQuery(this._queryTarget,
										'calculable')) {
									this._addDropBox(i);
								}
				...
				_buildSingleRadar : function(index) {
					...
					var name;
					var pointList;
					var calculable = this.deepQuery(this._queryTarget,
							'calculable');
					...
					for (var i = 0; i < data.length; i++) {
						...
						// 图例开关
						this.selectedMap[name] = legend ? legend
						...
						pointList = this._getPointList(this.serie.polarIndex,
								data[i]);
						// 添加拐点形状
						this._addSymbol(pointList, defaultColor, i, index,
								this.serie.polarIndex);
						// 添加数据形状
						this._addDataShape(pointList, defaultColor, data[i],
								index, i, calculable);
				...
				_getPointList : function(polarIndex, dataArr) {
					...
					for (var i = 0, l = dataArr.value.length; i < l; i++) {
						vector = polar.getVector(polarIndex, i,
								typeof dataArr.value[i].value != 'undefined'
										? dataArr.value[i].value
										: dataArr.value[i]);
						if (vector) {
				 ...
				 *            serieIndex
				 */
				_addSymbol : function(pointList, defaultColor, dataIndex,
						seriesIndex, polarIndex) {
					var series = this.series;
					var itemShape;
					var polar = this.component.polar;
								...
								.getSymbolShape(
										this
												.deepMerge([
																...
																series[seriesIndex]]),
										seriesIndex,
										series[seriesIndex].data[dataIndex].value[i],
										i,
										polar.getIndicatorText(polarIndex, i),
										pointList[i][0], // x
										pointList[i][1], // y
										this._symbol[this._radarDataCounter
												% this._symbol.length],
										defaultColor, '#fff', 'vertical');
						itemShape.zlevel = this._zlevelBase + 1;
						ecData.set(itemShape, 'data',
								series[seriesIndex].data[dataIndex]);
						ecData.set(itemShape, 'value',
				...
				_addDataShape : function(pointList, defaultColor, data,
					...
					var nColor = this.getItemStyleColor(this.deepQuery(
									queryTarget, 'itemStyle.normal.color'),
							seriesIndex, dataIndex, data);
					var nLineWidth = this.deepQuery(queryTarget,
							'itemStyle.normal.lineStyle.width');
					var nLineType = this.deepQuery(queryTarget,
							'itemStyle.normal.lineStyle.type');
					var nAreaColor = this.deepQuery(queryTarget,
							'itemStyle.normal.areaStyle.color');
					var nIsAreaFill = this.deepQuery(queryTarget,
							'itemStyle.normal.areaStyle');
					var shape = {
						...
						highlightStyle : {
							brushType : this.deepQuery(queryTarget,
									'itemStyle.emphasis.areaStyle')
									|| nIsAreaFill ? 'both' : 'stroke',
							...
							strokeColor : this.getItemStyleColor(
									...
									seriesIndex, dataIndex, data)
									|| nColor || defaultColor,
							lineWidth : this.deepQuery(queryTarget,
									'itemStyle.emphasis.lineStyle.width')
									|| nLineWidth,
							lineType : this.deepQuery(queryTarget,
					...
					ecData
							.pack(
									...
									this.component.polar
											.getIndicator(series[seriesIndex].polarIndex));
					if (calculable) {
				...
				_addDropBox : function(index) {
					var series = this.series;
					var polarIndex = this.deepQuery(this._queryTarget,
							'polarIndex');
				...
				ondragend : function(param, status) {
					...
					this.component.legend
							&& this.component.legend
									.del(	series[seriesIndex].data[dataIndex].name);
				...
				ondrop : function(param, status) {
					...
					if (dataIndex == -1) {
						...
						for (var i = 0; i < value.length; i++) {
							data.value[i] = accMath.accAdd(data.value[i],
									value[i]);
						}
