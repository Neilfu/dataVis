...
define(function(require) {
	...
	Bar.prototype = {
			...
			for (var position in _position2sIndexMap) {
				if (_position2sIndexMap[position].length > 0) {
					this._buildSinglePosition(position,
							_position2sIndexMap[position], this.xMarkMap);
				}
			}
			...
			switch (position) {
				...
				case 'top' :
					this._buildHorizontal(maxDataLength, locationMap,
							seriesArray, xMarkMap);
					break;
				case 'left' :
				case 'right' :
					this._buildVertical(maxDataLength, locationMap,
							seriesArray, xMarkMap);
					break;
			}
		},
				...
				if (legend) {
					...
					if (iconShape) {
						...
						if (serie.itemStyle.normal.borderWidth > 0) {
							...
							iconShape.style.strokeColor = iconShape.highlightStyle.strokeColor = serie.itemStyle.normal.borderColor;
			...
			for (var i = 0, l = maxDataLength; i < l; i++) {
				...
				for (var j = 0, k = locationMap.length; j < k; j++) {
					...
					for (var m = 0, n = locationMap[j].length; m < n; m++) {
						...
						if (i % interval === 0) {
							barShape = this._getBarItem(seriesIndex, i,
									categoryAxis.getNameByIndex(i), x, y,
					...
					for (var m = 0, n = locationMap[j].length; m < n; m++) {
						...
						if (this.deepQuery([data, serie, this.option],
							...
							barShape = this._getBarItem(seriesIndex, i,
									categoryAxis.getNameByIndex(i), x + 0.5, y
											+ 0.5,
									(barWidthMap[seriesIndex] || barWidth) - 1,
									this.ecTheme.island.r - 1, 'vertical');
					...
					for (var m = 0, n = locationMap[j].length; m < n; m++) {
						...
						if (i % interval === 0) {
							barShape = this._getBarItem(seriesIndex, i,
									...
									y - (barWidthMap[seriesIndex] || barWidth),
									barHeight, barWidthMap[seriesIndex]
											|| barWidth, 'horizontal');
							this.shapeList.push(new RectangleShape(barShape));
						}
					}
		 ...
		 * 
		 * @param {CategoryAxis}
		 *            categoryAxis 类目坐标轴，需要知道类目间隔大小
		 * @param {Array}
		 *            locationMap 整形数据的系列索引
		 */
		_mapSize : function(categoryAxis, locationMap, ignoreUserDefined) {
			...
			for (var j = 0, k = locationMap.length; j < k; j++) {
				...
				for (var m = 0, n = locationMap[j].length; m < n; m++) {
					...
					barMinHeightMap[seriesIndex] = this.query(queryTarget,
							'barMinHeight');
					barGap = typeof barGap != 'undefined' ? barGap : this
							.query(queryTarget, 'barGap');
					barCategoryGap = typeof barCategoryGap != 'undefined'
							? barCategoryGap
							: this.query(queryTarget, 'barCategoryGap');
				}
			}
			...
			if (locationMap.length != sBarWidthCounter) {
				...
				if (!ignoreUserDefined) {
					...
					if (typeof barGap == 'string' && barGap.match(/%$/)) {
						...
						barWidth = Math
								.floor(	(gap - sBarWidthTotal - barGap
												* (locationMap.length - 1))
												/ (locationMap.length - sBarWidthCounter));
					}
				...
				gap = sBarWidthCounter > 1
						? (typeof barCategoryGap == 'string' && barCategoryGap
								.match(/%$/))
								// 百分比
								? Math
										.floor(	categoryAxis.getGap()
		 ...
		 * 生成最终图形数据
		 */
		_getBarItem : function(seriesIndex, dataIndex, name, x, y, width,
				height, orient) {
			var series = this.series;
			var barShape;
			var serie = series[seriesIndex];
			...
			var normalColor = this.deepQuery(queryTarget,
					'itemStyle.normal.color')
					|| defaultColor;
			var emphasisColor = this.deepQuery(queryTarget,
			...
			barShape = {
				...
				style : {
					...
					height : height,
					brushType : 'both',
					color : this.getItemStyleColor(normalColor, seriesIndex,
							dataIndex, data),
				...
				highlightStyle : {
					color : this.getItemStyleColor(emphasisColor, seriesIndex,
							dataIndex, data),
			...
			barShape.highlightStyle.color = barShape.highlightStyle.color
					|| (typeof barShape.style.color == 'string' ? zrColor.lift(
							barShape.style.color, -0.3) : barShape.style.color);
			// 考虑线宽的显示优化
			...
			if (barShape.style.textPosition == 'insideLeft'
				...
				switch (barShape.style.textPosition) {
					case 'insideLeft' :
						...
						barShape.style.textY = barShape.style.y
								+ barShape.style.height / 2;
						barShape.style.textAlign = 'left';
						barShape.style.textBaseline = 'middle';
						break;
					case 'insideRight' :
						barShape.style.textX = barShape.style.x
								+ barShape.style.width - gap;
						barShape.style.textY = barShape.style.y
								+ barShape.style.height / 2;
			...
			ecData.pack(barShape, series[seriesIndex], seriesIndex,
					series[seriesIndex].data[dataIndex], dataIndex, name);
		...
		getMarkCoord : function(seriesIndex, mpData) {
			...
			if (mpData.type
				...
				dataIndex = typeof mpData.xAxis == 'string'
						&& xAxis.getIndexByName ? xAxis
						.getIndexByName(mpData.xAxis) : (mpData.xAxis || 0);

				var x = xMarkMap[dataIndex];
				x = typeof x != 'undefined'
						? x
						: typeof mpData.xAxis != 'string'
								&& xAxis.getCoordByIndex ? xAxis
								.getCoordByIndex(mpData.xAxis || 0) : xAxis
								.getCoord(mpData.xAxis || 0);

				pos = [x, yAxis.getCoord(mpData.yAxis || 0)];
			} else {
				// 纵向
				dataIndex = typeof mpData.yAxis == 'string'
						&& yAxis.getIndexByName ? yAxis
						.getIndexByName(mpData.yAxis) : (mpData.yAxis || 0);

				var y = xMarkMap[dataIndex];
				y = typeof y != 'undefined'
						? y
						: typeof mpData.yAxis != 'string'
								&& yAxis.getCoordByIndex ? yAxis
								.getCoordByIndex(mpData.yAxis || 0) : yAxis
								.getCoord(mpData.yAxis || 0);
				...
				if (aniMap[seriesIndex] && !aniMap[seriesIndex][3]) {
					...
					if (this.shapeList[i].type == 'rectangle') {
						...
						if (this.shapeList[i]._orient == 'horizontal') {
							// 条形图
							dy = this.component.yAxis.getAxis(serie.yAxisIndex
											|| 0).getGap();
							y = aniMap[seriesIndex][2] ? -dy : dy;
							x = 0;
						} else {
							// 柱形图
							dx = this.component.xAxis.getAxis(serie.xAxisIndex
											|| 0).getGap();
