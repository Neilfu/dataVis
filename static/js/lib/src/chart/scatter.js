	...
	Scatter.prototype = {
				...
				if (serie.type == ecConfig.CHART_TYPE_SCATTER) {
					...
					if (legend) {
						this.selectedMap[serieName] = legend
								.isSelected(serieName);

						this._sIndex2ColorMap[i] = zrColor.alpha(legend
										.getColor(serieName), 0.5);
						...
						if (iconShape) {
							...
							if (iconType.match('rectangle')) {
								iconShape.style.x += Math
										.round(	(iconShape.style.width - iconShape.style.height)
														/ 2);
								iconShape.style.width = iconShape.style.height;
							}

							if (iconType.match('star')) {
								iconShape.style.n = (iconType.replace('star',
										'') - 0)
										|| 5;
								iconType = 'star';
							}
		...
		_buildSeries : function(seriesArray) {
			...
			for (var j = 0, k = seriesArray.length; j < k; j++) {
				...
				this.xMarkMap[seriesIndex] = this._markMap(xAxis, yAxis,
						serie.data, pointList[seriesIndex]);
		...
		_markMap : function(xAxis, yAxis, data, pointList) {
			...
			for (var i = 0, l = pointList.length; i < l; i++) {
				/**
				 * x, // 横坐标 y, // 纵坐标 i, // 数据index data.name || '' // 名称
				 */
			...
			xMarkMap.average0 = (xMarkMap.sum0 / xMarkMap.counter0).toFixed(2)
					- 0;
			var x = xAxis.getCoord(xMarkMap.average0);
			// 横轴平均纵向
			xMarkMap.averageLine0 = [[x, gridYend], [x, gridY]];
			xMarkMap.minLine0 = [[xMarkMap.minX0, gridYend],
					[xMarkMap.minX0, gridY]];
			xMarkMap.maxLine0 = [[xMarkMap.maxX0, gridYend],
					[xMarkMap.maxX0, gridY]];

			xMarkMap.average1 = (xMarkMap.sum1 / xMarkMap.counter1).toFixed(2)
					- 0;
			var y = yAxis.getCoord(xMarkMap.average1);
			// 纵轴平均横向
			xMarkMap.averageLine1 = [[gridX, y], [gridXend, y]];
			xMarkMap.minLine1 = [[gridX, xMarkMap.minY1],
					[gridXend, xMarkMap.minY1]];
			xMarkMap.maxLine1 = [[gridX, xMarkMap.maxY1],
		...
		_buildPointList : function(pointList) {
			...
			for (var seriesIndex in pointList) {
				...
				if (serie.large && serie.data.length > serie.largeThreshold) {
					this.shapeList.push(this._getLargeSymbol(seriesPL, this
									.getItemStyleColor(this.query(serie,
				...
				/*
				 * pointlist=[ 0 x, 1 y, 2 数据index 3 名称 ]
				 */
		...
		_getSymbol : function(seriesIndex, dataIndex, name, x, y) {
			...
			var itemShape = this.getSymbolShape(serie, seriesIndex, data,
					...
					rangColor, 'rgba(0,0,0,0)', 'vertical');
		...
		getMarkCoord : function(seriesIndex, mpData) {
			...
			if (mpData.type
				...
				var valueIndex = typeof mpData.valueIndex != 'undefined'
						? mpData.valueIndex
						: 1;
				pos = [xMarkMap[mpData.type + 'X' + valueIndex],
