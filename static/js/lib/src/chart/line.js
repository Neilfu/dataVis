	...
	Line.prototype = {
		...
		_buildShape : function() {
			...
			for (var position in _position2sIndexMap) {
				if (_position2sIndexMap[position].length > 0) {
					this._buildSinglePosition(position,
							_position2sIndexMap[position]);
				}
			}
			this.addShapeList();
		},
		...
		_buildSinglePosition : function(position, seriesArray) {
			...
			switch (position) {
				case 'bottom' :
				case 'top' :
					this._buildHorizontal(seriesArray, maxDataLength,
							locationMap, this.xMarkMap);
					break;
				case 'left' :
				case 'right' :
					this._buildVertical(seriesArray, maxDataLength,
							locationMap, this.xMarkMap);
					break;
			}
		...
		_mapData : function(seriesArray) {
			...
			for (var i = 0, l = seriesArray.length; i < l; i++) {
				...
				if (legend) {
					...
					this._sIndex2ColorMap[seriesArray[i]] = legend
							.getColor(serieName);
					...
					this._sIndex2ColorMap[seriesArray[i]] = this.zr
							.getColor(seriesArray[i]);
				}
			...
			/*
			 * 调试输出 var s = ''; for (var i = 0, l = maxDataLength; i < l; i++) {
			 * s = '['; for (var j = 0, k = locationMap.length; j < k; j++) { s
			 * +='[' for (var m = 0, n = locationMap[j].length - 1; m < n; m++) {
			 * s += series[locationMap[j][m]].data[i] + ',' } s +=
			 * series[locationMap[j][locationMap[j].length - 1]] .data[i]; s +=
			 * ']' } s += ']'; console.log(s); } console.log(locationMap)
			 */
			...
			for (var i = 0, l = maxDataLength; i < l; i++) {
				...
				for (var j = 0, k = locationMap.length; j < k; j++) {
					...
					for (var m = 0, n = locationMap[j].length; m < n; m++) {
						...
						curPLMap[seriesIndex].push([x, y, i,
										categoryAxis.getNameByIndex(i), x,
										baseYP]);
				...
				for (var j = 0, k = locationMap.length; j < k; j++) {
					for (var m = 0, n = locationMap[j].length; m < n; m++) {
						...
						if (this.deepQuery([data, serie, this.option],
								'calculable')) {
							symbolSize = this.deepQuery([data, serie],
									'symbolSize');
							lastYP += symbolSize * 2 + 5;
							y = lastYP;
							this.shapeList.push(this._getCalculableItem(
									seriesIndex, i, categoryAxis
											.getNameByIndex(i), x, y,
									'horizontal'));
						}
					}
				}
			...
			for (var j = 0, k = locationMap.length; j < k; j++) {
				for (var m = 0, n = locationMap[j].length; m < n; m++) {
					...
					if (xMarkMap[seriesIndex].counter > 0) {
						xMarkMap[seriesIndex].average = (xMarkMap[seriesIndex].sum / xMarkMap[seriesIndex].counter)
								.toFixed(2)
								- 0;
					}
					y = this.component.yAxis
					...
					xMarkMap[seriesIndex].minLine = [
							[this.component.grid.getX(),
									xMarkMap[seriesIndex].minY],
							[this.component.grid.getXend(),
									xMarkMap[seriesIndex].minY]];
					xMarkMap[seriesIndex].maxLine = [
							[this.component.grid.getX(),
									xMarkMap[seriesIndex].maxY],
							[this.component.grid.getXend(),
									xMarkMap[seriesIndex].maxY]];
				}
			}

			this._buildBorkenLine(seriesArray, this.finalPLMap, categoryAxis,
					'horizontal');
		},
		 ...
		 * 构建类目轴为垂直方向的折线图系列
		 */
		_buildVertical : function(seriesArray, maxDataLength, locationMap,
				xMarkMap) {
			var series = this.series;
			// 确定类目轴和数值轴，同一方向随便找一个即可
			var seriesIndex = locationMap[0][0];
			...
			for (var i = 0, l = maxDataLength; i < l; i++) {
				...
				for (var j = 0, k = locationMap.length; j < k; j++) {
					...
					for (var m = 0, n = locationMap[j].length; m < n; m++) {
						...
						curPLMap[seriesIndex].push([x, y, i,
										categoryAxis.getNameByIndex(i), baseXP,
										y]);
		 ...
		 * 生成折线和折线上的拐点
		 */
		_buildBorkenLine : function(seriesArray, pointList, categoryAxis,
				orient) {
			var series = this.series;
			var defaultColor;
			...
			for (var sIdx = seriesArray.length - 1; sIdx >= 0; sIdx--) {
				...
				if (serie.type == this.type && typeof seriesPL != 'undefined') {
					...
					// 多级控制
					lineWidth = this.query(serie,
							'itemStyle.normal.lineStyle.width');
					lineType = this.query(serie,
							'itemStyle.normal.lineStyle.type');
					lineColor = this.query(serie,
							'itemStyle.normal.lineStyle.color');
					normalColor = this.getItemStyleColor(this.query(serie,
									'itemStyle.normal.color'), seriesIndex, -1);

					isFill = typeof this.query(serie,
							'itemStyle.normal.areaStyle') != 'undefined';

					fillNormalColor = this.query(serie,
					...
					for (var i = 0, l = seriesPL.length; i < l; i++) {
						...
						if (!isLarge) { // 非大数据模式才显示拐点symbol
							for (var j = 0, k = singlePL.length; j < k; j++) {
								...
								if (this.deepQuery([data, serie],
										'showAllSymbol') // 全显示
										|| (categoryAxis
												.isMainAxis(singlePL[j][2]) && this
												.deepQuery([data, serie],
														'symbol') != 'none') // 主轴非空
										|| this.deepQuery([data, serie,
														this.option],
												'calculable') // 可计算
								) {
							...
							// 大数据模式截取pointList
							singlePL = this
									._getLargePointList(orient, singlePL);
						}
						...
						brokenLineShape = new BrokenLineShape({
							...
							style : {
								...
								pointList : singlePL,
								strokeColor : lineColor || normalColor
										|| defaultColor,
								lineWidth : lineWidth,
								lineType : lineType,
						...
						ecData.pack(brokenLineShape, series[seriesIndex],
								seriesIndex, 0, i, series[seriesIndex].name);
						...
						if (isFill) {
							...
							ecData.pack(halfSmoothPolygonShape,
									series[seriesIndex], seriesIndex, 0, i,
		...
		_isLarge : function(orient, singlePL) {
			if (singlePL.length < 2) {
				...
				return orient == 'horizontal' ? (Math.abs(singlePL[0][0]
						- singlePL[1][0]) < 0.5) : (Math.abs(singlePL[0][1]
						- singlePL[1][1]) < 0.5);
			}
		},
		...
		_getSmooth : function(isSmooth/* , pointList, orient */) {
			if (isSmooth) {
				/*
				 * 不科学啊，发现0.3通用了 var delta; if (orient == 'horizontal') { delta =
				 * Math.abs(pointList[0][0] - pointList[1][0]); } else { delta =
				 * Math.abs(pointList[0][1] - pointList[1][1]); }
				 */
				return 0.3;
			} else {
				return 0;
		 ...
		 * 生成空数据所需的可计算提示图形
		 */
		_getCalculableItem : function(seriesIndex, dataIndex, name, x, y,
				orient) {
			var series = this.series;
			var color = series[seriesIndex].calculableHolderColor
					|| this.ecTheme.calculableHolderColor;

			var itemShape = this._getSymbol(seriesIndex, dataIndex, name, x, y,
					orient);
		...
		_getSymbol : function(seriesIndex, dataIndex, name, x, y, orient) {
			...
			var itemShape = this.getSymbolShape(serie, seriesIndex, data,
					dataIndex, name, x, y, this._sIndex2ShapeMap[seriesIndex],
					this._sIndex2ColorMap[seriesIndex], '#fff',
		...
		getMarkCoord : function(seriesIndex, mpData) {
			...
			if (mpData.type
				...
				// 特殊值内置支持
				return [xMarkMap[mpData.type + 'X'],
						xMarkMap[mpData.type + 'Y'],
						xMarkMap[mpData.type + 'Line'], xMarkMap[mpData.type]];
			}
					...
					if (this.shapeList[i]._main) {
						...
						if (aniMap[seriesIndex][2]) {
							...
							if (this.shapeList[i].type == 'polygon') {
								...
								this.shapeList[i].style.pointList[len - 3] = pointList[len
										- 2];
								isHorizontal
										? (this.shapeList[i].style.pointList[len
												- 3][0] = pointList[len - 4][0])
										: (this.shapeList[i].style.pointList[len
												- 3][1] = pointList[len - 4][1]);
								this.shapeList[i].style.pointList[len - 2] = pointList[len
										- 1];
							}
							...
							if (this.shapeList[i].type == 'polygon') {
								// 区域图
								var targetPoint = this.shapeList[i].style.pointList
										.pop();
								isHorizontal
										? (targetPoint[0] = pointList[0][0])
										: (targetPoint[1] = pointList[0][1]);
								this.shapeList[i].style.pointList
										.push(	targetPoint);
							}
							isHorizontal ? (x = -dx, y = 0) : (x = 0, y = dy);
						}
						...
						// 拐点动画
						if (aniMap[seriesIndex][2]
								&& this.shapeList[i]._dataIndex == series[seriesIndex].data.length
										- 1) {
							// 队头加入删除末尾
